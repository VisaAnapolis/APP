...
uses
  ...
  jpeg, dateutils, StrUtils, ImgList, ExtDlgs, OleServer, Word2000, OleCtrls, SHDocVw,
...
type
  Tfrmestabe = class(TForm)
    ...
    BtAPICSV: TButton;
    tbtaxa: TTable;
    tbtaxaCONTROLE: TAutoIncField;
    tbtaxaINSCRICAO: TStringField;
    tbtaxaEXERCICIO: TSmallintField;
    tbtaxaVALOR: TFloatField;
    tbtaxaDT_VENC: TDateField;
    tbtaxaSITUACAO: TStringField;
    tbtaxaPAGO: TBooleanField;
    tbtaxaCOMPLEXIDADE: TStringField;
    tbtaxaAREA: TFloatField;
    dstaxa: TDataSource;
    DBEdit42: TDBText;
    DBEdit45: TDBText;
    DBEdit46: TDBText;
...
{    filtro:integer;}
    ...
    FSimEndereco: TLabel;
    FSimRazao: TLabel;
    FLastSIMRazao: string;
    FTSimAux: TTable;
    FTSimAuxTemIndice: Boolean;
    FLastSIMChave: string;
    FLastSIMEndereco: string;
    ...
    function CaminhoSIMCad: string;
    function CaminhoSIMCnae: string;
    function CaminhoSIMAuxCSV: string;

    function CaminhoTaxaCSV: string;
    function CaminhoTaxaTabela: string;
    function ImportarTaxaSIM: Boolean;

    function TabelaEmUso(const NomeTabela: string): Boolean;

    procedure AtualizarLbTimestamp;

    procedure CarregarTaxaSIM;
     procedure CarregarEnderecoSIM;

    function PrecisaGerarSIMTabela: Boolean;

    function GerarSIMTabela: Boolean;
    function ExportarSIMAUXParaCNAEAuxCSV: Boolean;
    procedure ValidarCNAESIM_Tabela;

    function ExisteDivergenciaCNAE: Boolean;
    function CNAEsAlvlibForaRT: string;
...
const
  { Inatividade: tempo em segundos sem atividade para fechar o form. Teste: 30; producao (meia hora): 1800 }
  INATIVIDADE_SEGUNDOS = 900;
...
//  if Assigned(TimerInatividade) then TimerInatividade.Enabled := True;
end;

procedure Tfrmestabe.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//  if Assigned(TimerInatividade) then TimerInatividade.Enabled := False;
lugar := 0;
frmlocpro.inclu:=0;
tbtaxa.Close;
if Assigned(FTSimAux) then begin FTSimAux.Close; FreeAndNil(FTSimAux); end;
tbos.close;
tbalvrt.Close;
tbsinavisa.close;
tbhistorico.close;
...
{    if (tbsinavisacomplexidade.value <> complex) and (complex <> '' ) and (frmlogin.c_grp <> 'ADM') then
         begin
            showmessage('Alteração da Complexidade do Contribuinte Não autorizada para o usuário.');
            DBLGrupo.setfocus;
            exit;
      end;}
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' já cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Usuário Não autorizado',mtwarning,[mbok],0);
//            if messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' já cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicação do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
           exit;
          end
          else if messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' já cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicação do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
       end;
    end;
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CPF: '+tbauxcontribcpf.asstring+' já cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Perfil Não autorizado',mtwarning,[mbok],0);
          exit;
         end
         else if messagedlg('CPF: '+tbauxcontribcpf.asstring+' já cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicação do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
       end;
    end;
        ...
        begin
            showmessage('Caro Usuário, favor informar a Razão Social');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuário, favor informar o tipo de Cadastro');
            exit;
        end;
        ...
        if tbcontribpessoa.value = 'CNPJ' then
            begin
                ...
                begin
                    showmessage('Caro Usuário, favor informar o CNPJ');
                    exit;
                end;
                tbcontribcpf.value := '';
            ...
            begin
                ...
                begin
                    showmessage('Caro Usuário, favor informar o CPF');
                    exit;
                end;
                tbcontribcgc.value := '';
        ...
        begin
            showmessage('Caro Usuário, favor informar o Logradouro');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuário, favor informar o Endereço Completo');
            exit;
        end;

        if tbcontribhorario.value = '' then
        begin
            showmessage('Caro Usuário, favor informar o Período de Funcionamento');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuário, favor Selecionar um Bairro');
            exit;
        end;

{        if tbcontribhorario.IsNull then
        begin
            showmessage('Caro Usuário, favor informar o horário de funcionamento');
            exit;
        end;}
...

{
        if (tbcontribarea.value <> tbgrupoarea.value) and (tbcontribinatividade.Value <> true) then
          begin
           showmessage('Caro Usuário, Grupo Econômico selecionado divergente da área de Fiscalização');
           exit;
        end;
        if (tbcontribgrupo.value <> tbativigrupo.value) and (tbcontribinatividade.Value <> true) then
          begin
           showmessage('Caro Usuário, Atividade Econômica selecionada divergente do Grupo Econômico');
           exit;
        end;

        if (tbcontribarea.value = 0) and (tbcontribinatividade.Value <> true) then
        begin
            showmessage('Caro Usuário, favor Selecionar a área de Fiscalização relacionada');
            exit;
        end;

        if (tbcontribgrupo.value = 0) and (tbcontribinatividade.Value <> true) then
        begin
            showmessage('Caro Usuário, favor Selecionar o Grupo Econômico relacionado');
            exit;
        end;}

{        if tbcontribatividade.value = 0 then
        begin
            showmessage('Caro Usuário, favor Selecionar a Atividade Econômica relacionada');
            exit;
        end;    }  
...
{  vasio:= false;
  ...
  begin
      ...
      if vasio = true then
      begin
          showmessage('Caro Usuário, favor completar a ficha CAE para todas as atividades cadastradas');
          exit;
      end;
      tbrt.Next;
  end;
        ...
        begin
            showmessage('Caro Usuário, favor preencher a ficha CAE');
            exit;
        end;}
...
{        end;
        ...
        begin
               if not (tbrt.eof) and not (tbcaracter.eof) then
                begin
                     showmessage('Atenção! Para salvar as Alterações nas "ESPECIFICAÇÕES", marque a opção "RT/CARACTERIZAÇÃO".');
                     exit;
                end;
        end;
...
case key of
                ...
                if tbalvara.state<>dsbrowse then btgravalvaraClick(Sender) else
                    begin
                        if tbcontrib.state<>dsbrowse then
                              ...
                              showmessage('F6 Não suportada em modo consulta');
                    end;
        ...
        begin
            if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F3 Não suportada em modo edição');
        end;
        vk_F2:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F2 Não suportada em modo edição');
        end;
        vk_F1:
        begin
         if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F1 Não suportada em modo edição');
        end;
        vk_F4:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F4 Não suportada em modo edição');
        end;
        vk_F5:
        begin
        if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                    PageControl1.ActivePage := TabAlvara
                      ...
                      showmessage('F5 Não suportada em modo edição');
        end;
...
//        tbvisitaslibera.Value := 'Não Liberado';
        DbCordem.setfocus;
{        tbvisitas.last;
...
begin
     ...
     if messagedlg('Confirma a Exclusão do Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
...
retorno := false;
      perform(Wm_nextdlgctl,0,0);
        ...
        begin
                showmessage('Caro Usuário, favor informar Ação Fiscal');
                exit;
        end
        else
        begin
                if  tbvisitasACAO.Value= 'ATENDIMENTO a DENÚNCIA' then
                begin
                        if tbvisitasDenuncia.value = 0 then
                        begin
                                showmessage('Caro Usuário, favor informar Número da Denúncia');
                                exit;
                        end;
                end;
        end;
        ...
        begin
            showmessage('Caro Usuário, favor informar qual Documento Emitido');
            exit;
        end;
        if tbvisitasdt_visita.isnull then
        begin
            showmessage('Caro Usuário, favor informar a data do Documento' );
            DBEDataV.setfocus;
            exit;
        end;
        if (tbvisitasatividade.IsNull) and (frmlogin.c_grp = 'FIS') then
        begin
            showmessage('Caro Usuário, favor informar a atividade inspecionada' );
            DBEDataV.setfocus;
            exit;
        end;
        

        if (tbvisitastipo.Value = 'TERMO DE INTIMAÇÃO') or (tbvisitastipo.Value = 'TERMO DE NOTIFICAÇÃO') or (tbvisitastipo.Value = 'CERTIDÃO') then
        begin
            if (tbvisitaslibera.value <> 'Sim') and (tbvisitaslibera.value <> 'Não') then
            begin
                showmessage('Caro Usuário, favor informar se a OS necessita retorno' );
                DBCLibera.setfocus;
                exit;
            end;
        end;
        ...
        begin
            ...
            begin
                showmessage('Caro Usuário, favor informar a origem da OS' );
                DBCOrdem.setfocus;
                exit;
            end;
        end;
       ...
       if (tbvisitasmodalidade.value <> 'REQUERIMENTO') and (tbvisitasmodalidade.value <> 'DENÚNCIA') and (tbvisitasmodalidade.value <> 'DE OFÍCIO') and (tbvisitasmodalidade.value <> 'PROTOCOLO') then
       begin
           if (frmlogin.c_grp = 'ADM') or (frmlogin.c_per = 'JUR') then
           else
           begin
                showmessage('Caro Usuário, favor selecionar uma das opções na origem da OS' );
                DBCOrdem.setfocus;
                exit;
           end;
       end;
        ...
        begin
        showmessage('Caro Usuário, favor informar o número da OS de requerimento' );
                DBEOs.setfocus;
                exit;
        end;

        if (tbvisitasmodalidade.value = 'DENÚNCIA') and (tbvisitasdenuncia.IsNull) then
        begin
                showmessage('Caro Usuário, favor informar o número da denúncia' );
                DBEDen.setfocus;
                exit;
        end;
        if (tbvisitasmodalidade.value = 'PROTOCOLO') and (tbvisitasprotocolo.IsNull) then
        begin
                showmessage('Caro Usuário, favor informar o número do Protocolo' );
                DBEProto.setfocus;
                exit;
        end;
        if (tbvisitasmodalidade.value = 'DE OFÍCIO') and (tbvisitasoficio.IsNull) then
        begin
                showmessage('Caro Usuário, favor informar o número da OS De Ofício.' );
                DBEOf.setfocus;
                exit;
        end;
...
//===============================================================================================
 { cae:= false;
  ...
  if cae = false then
  begin
      showmessage('Atividade Econômica inexistente na ficha CAE do contribuinte!');
      exit;
  end;
   }
//===============================================================================================
        ...
        if (tbrmpfnome.value=tbvisitasfiscal1.value) or (tbrmpfnome.value=tbvisitasfiscal2.value) or (tbrmpfnome.value=tbvisitasfiscal3.value) then
           begin
               if (tbvisitasdt_visita.value=tbrmpfdata.value) then
                  begin
                     messagedlg('Relatório de Produção Homologado: '+datetostr(tbrmpfdata.value)+' - '+tbrmpfnome.value+'.',mtwarning,[mbok],0);
                     exit;
                  end;
           end;
        tbrmpf.Next;
   ...
   begin
       if dayofweek(date) = 2 then
        begin
         ...
         begin
            showmessage('Impossível alterar a inspeção de '+ datetostr(datavisita) +' para '+datetostr(tbvisitasdt_visita.value));
            DBEDataV.setfocus;
            exit;
         end;
       end
       ...
       begin
        ...
        if (tbvisitasnumero.isnull) and (tbvisitastipo.value <> 'CERTIDÃO')  then
        begin
        ...
        begin
            showmessage('Caro Usuário, favor informar Pelo menos uma Autoridade Emitente');
            exit;
        end;
...
{        if not (tbvisitasfiscal3.IsNull) and not (tbvisitasfiscal2.isnull) and not (tbvisitasfiscal3.isnull) and not tbvisitasporte = 'GRANDE' then
        begin
            showmessage('Não permitido 3 (três) autoridades por inspeção!');
            exit;
        end;
 }
        if tbvisitastipo.value = 'ATO ADMINISTRATIVO DA GERÊNCIA' then
        begin
       ...
       if (tbvisitastipo.value <> 'ATO ADMINISTRATIVO DA GERÊNCIA') and (frmlogin.c_grp = 'CAD' ) then
       begin
             if messagedlg('Atenção!!'+#13+tbvisitastipo.Value+' é documento fiscal. Confirma a inclusão da peça emitida pelo fiscal '+tbvisitasfiscal1.Value+' em caráter excepcional, objetivando a atualização cadastral ?',mtconfirmation,[mbyes,mbno],0)=mrno then exit;
       end;


//**************************
{if (tbvisitastipo.Value<>'RELATORIO TECNICO') and (tbvisitastipo.Value<>'CERTIDÃO') and (tbvisitastipo.Value<>'CONTRA RAZAO') and (tbvisitastipo.Value<>'PRORROGACAO') and (tbvisitastipo.Value<>'REPLICA FISCAL') and (tbvisitastipo.Value<>'ATO ADMINISTRATIVO DA GERÊNCIA') then
begin

       if tbvisitash_fim.value < tbvisitash_visita.value  then
        begin
              showmessage('Final da Inspeção Anterior ao início');
              exit;
        end;
 ...
 if (tbvisitastipo.value = 'AUTO DE IMPOSIÇÃO DE PENALIDADE') or
    (tbvisitastipo.value = 'AUTO DE INFRAÇÃO')                or
    (tbvisitastipo.value = 'TERMO DE INTIMAÇÃO')              or
    (tbvisitastipo.value = 'TERMO DE NOTIFICAÇÃO')
 then
 begin

{       if tbvisitash_visita.isnull  then
        begin
              showmessage('Favor informar hora do início da inspeção');
              exit;
        end;

         if tbvisitash_fim.isnull  then
        begin
              showmessage('Favor informar hora do final da inspeção');
              exit;
        end;
 end;
}
...
{   if (tbvisitasporte.isnull) and (tbvisitasacao.value='INSPECAO SANITARIA') then
   begin
        showmessage('Favor informar o porte da inspeção');
        DBCporte.setfocus;
        exit;
   end;    }
...
//     if (daysbetween(date,tbvisitasdt_visita.value) > 9) and (frmlogin.c_grp = 'FIS') then
     ...
     begin
        showmessage('Data retroativa para inclusão/alteração de inspeções!');
        DBEDataV.setfocus;
        exit;
     end;
   end
...
//**********************************************************
        ...
        begin
                if (tbvisitastipo.Value = 'TERMO DE NOTIFICAÇÃO')            or
                   (tbvisitastipo.Value = 'TERMO DE INTIMAÇÃO')              or
                   (tbvisitastipo.Value = 'TERMO DE COLETA DE AMOSTRA')      or
                   (tbvisitastipo.Value = 'CERTIDÃO')                        or
                   (tbvisitastipo.Value = 'AUTO DE INFRAÇÃO')                or
                   (tbvisitastipo.Value = 'AUTO DE IMPOSIÇÃO DE PENALIDADE') then
               begin
                    showmessage('Caro Usuário, obrigatório informar meio de locomoção para a atividade informada' );
                    DBCMeio.SetFocus;
                    exit;
               end;
        end;

        if (tbvisitasmeio.value = 'Proprio') and (tbvisitastipo.Value <> 'TERMO DE NOTIFICAÇÃO')            and
                                                 (tbvisitastipo.Value <> 'TERMO DE INTIMAÇÃO')              and
                                                 (tbvisitastipo.Value <> 'TERMO DE COLETA DE AMOSTRA')      and
                                                 (tbvisitastipo.Value <> 'CERTIDÃO')                        and
                                                 (tbvisitastipo.Value <> 'AUTO DE INFRAÇÃO')                and
                                                 (tbvisitastipo.Value <> 'AUTO DE IMPOSIÇÃO DE PENALIDADE') then
        begin
            showmessage('Caro Usuário, meio de locomoção incompatível com a atividade informada' );
            DBCMeio.SetFocus;
            exit;
        end;

{
ATO ADMINISTRATIVO DA GERÊNCIA
CONFERENCIA DE BALANão
 ...
 //*******************************************
             ...
             begin
                ...
                begin
                     ...
                     begin
                        messagedlg('Informações inconsistentes com outra entrada da tabela de inspeções. Autoridade: '+tbauxvisitafiscal1.value+', Inscrição: '+tbauxvisitacodigo.asstring+', Período: '+tbauxvisitah_visita.asstring+' as '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' na '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
//**********************************************
             ...
             begin
                ...
                begin
                     if (tbvisitash_visita.value > tbauxvisitah_visita.value) and (tbvisitash_visita.value < tbauxvisitah_fim.value) then
                     begin
                        messagedlg('Informações inconsistentes com outra entrada da tabela de inspeções. Autoridade: '+tbauxvisitafiscal2.value+', Inscrição: '+tbauxvisitacodigo.asstring+', Período: '+tbauxvisitah_visita.asstring+' as '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' na '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
//**********************************************
             ...
             begin
                ...
                begin
                     if (tbvisitash_visita.value > tbauxvisitah_visita.value) and (tbvisitash_visita.value < tbauxvisitah_fim.value) then
                     begin
                        messagedlg('Informações inconsistentes com outra entrada da tabela de inspeções. Autoridade: '+tbauxvisitafiscal3.value+', Inscrição: '+tbauxvisitacodigo.asstring+', Período: '+tbauxvisitah_visita.asstring+' as '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' na '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
{     end;}
       ...
       begin
           ...
           showmessage('Atenção! Terceiro fiscal Não autorizado para esta OS');
           dbcordem.SetFocus;
           exit
       end;
                      ...
                      begin
                          if (tbvisitastipo.Value <> 'CERTIDÃO') or (tbreqcomplexidade.Value = 'BAIXA') then
                          begin
                              tbreq.Edit;
                              ...
                              tbreq.Post;
                              showmessage('Ordem de serviço: ' + tbreqos.AsString+' Baixada com sucesso.');
                          end
                          else
                          begin
                              tbvisitaslibera.Value := 'Não';
                              showmessage('Atenção! A Ordem de Requerimento: ' + tbreqos.AsString+' Não foi baixada pela certidão emitida. Necessário reportar a Gerência');
                          end;
                      end
                      else
                      begin
                          if tbvisitastipo.Value <> 'CERTIDÃO' then
                          showmessage('Tudo bem! A OS '+ tbreqos.AsString+' já foi baixada anteriormente.' )
                          else
                          begin
                              if tbreqcomplexidade.Value <> 'BAIXA' then
                              begin
                                  showmessage('Não pode ser emitida certidão para Ordem de Requerimento: ' + tbreqos.AsString+' já baixada.');
                                  DBCtipo.SetFocus;
                                  exit;
                              end;
                          end;
                    ...
                    begin
                        showmessage('Atenção. OS encaminhada para '+tbreqfiscal_encaminha.Value );
                        dbeos.SetFocus;
                        exit;
                    end;
                  end
                  ...
                  begin
                    showmessage('Requerimento ainda Não é uma OS encaminhada pelo titular de vigilância');
                    dbeos.SetFocus;
                    exit;
                  end;
                end
                ...
                begin
                    showmessage('OS Não pertence a este regulado' );
                    dbeos.SetFocus;
                    exit;
                end;
            end
        ...
        begin
            ...
            begin
                ...
                begin
                    showmessage('Atenção! OS Cancelada '+ tboficiooficio.AsString);
                    dbeof.SetFocus;
                    exit;
                end;
                ...
                begin
                    showmessage('Atenção! Impossível registrar nesta data. A OS informada foi encaminhada em:'+ tboficiodtencaminha.AsString);
                    DBEDataV.SetFocus;
                    exit;
                end;
       ...
       begin
           ...
           exit
                ...
                begin
                    ...
                    begin
                        showmessage('Atenção! OS encaminhada ao fiscal '+tboficiofiscalencaminha.Value+'. Impossível continuar');
                        dbeof.SetFocus;
                        exit;
                    end;
                    if (tboficiomotivo.Value = 'SERVIÇO TÉCNICO') or (tboficiomotivo.Value = 'OPERAÇÃO FISCAL')  or (tboficiomotivo.Value = 'TREINAMENTO/REUNIAO')  then
                    begin
                        showmessage('Atenção! OS Não direcionada a inspeção. Impossível continuar');
                        dbeof.SetFocus;
                        exit;
                    end;
                    ...
                    begin
                        ...
                        tboficio.Post;
                        showmessage('Ordem de serviço: ' + tboficiooficio.AsString+' Baixada com sucesso.');
                    end
                    else
                    begin
                        showmessage('Atenção! OS '+ tboficiooficio.AsString+' já baixada anteriormente.' );
                    end;
                end
                else
                begin
            ...
            begin
                ...
                begin
                    showmessage('N. de protocolo Não pertence ao regulado.');
                    dbeproto.SetFocus;
                    exit;
                end
//                else  showmessage('Achei protocolo '+inttostr(tbauxplantaprotocolo.value)+'  '+'regulado '+inttostr(tbauxplantacodigo.value))
            ...
            begin
                ...
                begin
                    ...
                    begin
                        showmessage('Atenção! Denúncia Arquivada:'+ tbdenunciadenuncia.AsString);
                        dbeden.SetFocus;
                        exit;
                    end;
                    if tbdenunciafiscalencaminha.Value <> frmlogin.c_user then
                    begin
                        showmessage('Atenção! Denúncia encaminhada para o fiscal:'+ tbdenunciafiscalencaminha.Value );
                        dbeden.SetFocus;
                        exit;
                    end;
                end
                ...
                begin
                    showmessage('N. de denúncia Não encontrado.');
                    dbeden.SetFocus;
                    exit;
                end;
            end;
...
//======================================================
              ...
              if (tbvisitasmodalidade.Value='DENÚNCIA')     and (tbauxoficio.findkey(['DE'+inttostr(tbvisitasdenuncia.value)])) then retorno := true;
              if (tbvisitasmodalidade.Value='DE OFÍCIO')    and (tbauxoficio.findkey(['OF'+inttostr(tbvisitasoficio.value)]))   then retorno := true;

              if retorno = false then
              begin
                  if tbvisitaslibera.Value = 'Sim' then
                  begin
                      if messagedlg('Confirma a geração automática do Retorno para a OS '+tbvisitasos.AsString+tbvisitasoficio.AsString+tbvisitasdenuncia.AsString+' ?',mtconfirmation,[mbyes,mbno],0)=mryes  then
                      begin
                          if  (tbvisitasmodalidade.value <> 'PROTOCOLO') then
                          begin
                                tboficio.Insert;
                                ...
                                if tbvisitasmodalidade.value = 'DENÚNCIA'  then
                                    begin
                                        ...
                                        tboficioordem.value := 'Retorno da Denúncia '+tbvisitasdenuncia.AsString;
                                        tboficiofiscalencaminha.Value := tbdenunciafiscalencaminha.Value;
                                    end;
                                if tbvisitasmodalidade.value = 'DE OFÍCIO' then
                                    begin
                                       tboficioorigem.value := 'OF'+inttostr(tbvisitasoficio.Value);
                                       tboficioordem.value := 'Retorno da Ordem de Ofício '+tbvisitasoficio.AsString;
              ...
              else showmessage('Atenção! Retorno para OS ' + tbvisitasos.AsString + tbvisitasoficio.AsString + tbvisitasdenuncia.AsString + ' já foi gerado anteriormente.');
//=================================================
{
        ...
        end;
{
        ...
        begin
            showmessage('Caro Usuário, favor informar o CNAE na ficha CAE');
            exit;
        end;
 }
  ...
  {       if tbcontribatividade.value = 0 then
    ...
    begin
      ...
      begin
         ...
         begin
            messagedlg('Atenção! Reportar-se à gerência devido a apontamento nesta data : '+datetostr(tbrdpfdata.value)+' - '+tbrdpfnome.value,mtwarning,[mbok],0);
            exit;
         end;
      end;
      tbrdpf.next;
...
{     if tbvisitasentrega.value = true then
     begin
        showmessage('Este documento já está anexado a um Relatório de Produção Fiscal já entregue à Administração. Impossível Alterar!');
        exit;
     end; }
   ...
   begin
       if dayofweek(date) = 2 then
        begin
         ...
         begin
            showmessage('Perfil Não autorizado alterar documento nesta data!');
...
{(tbvisitastipo.value='ATO ADMINISTRATIVO DA GERÊNCIA')}
then
begin
...
showmessage('Perfil Não autorizado!');
end;
...
//    =======================
     ...
     if messagedlg('Confirma a Exclusão do Documento N..:'+#13+ tbvisitasnumero.AsString + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
            ...
            if (daysbetween(date,tbvisitasdt_visita.value) > 1) then
               begin
                 showmessage('Impossível excluir a inspeção nesta data!');
                 DBEDataV.setfocus;
                 exit;
               end
               else
               begin
                    if not tbhistorico.Eof then tbhistorico.Delete else showmessage('Especificações Não digitadas para este Documento!');
                    if not tbvisitas.Eof   then tbvisitas.Delete   else showmessage('Não Há Documento a ser Excluído!');
                    Showmessage('Atenção. Lançamento excluído. É necessário gerar novamente o Relatório de Produção para esta data');
               end;
             end
             else showmessage('Perfil Não autorizado!');
     end;
...
begin
 showmessage('Alvará Não requerido para o exercício!');
 exit;
end;

if tbalvaralibera.value <> true then
begin
 showmessage('Alvará Não liberado para o exercício!');
 exit;
end;
painel:= 'alvara';
...
begin
 ...
 //      if messagedlg('Confirma a Requerimento de multiplos alvaras do contribuinte para o exercicio: '+tbalvaraexercicio.AsString +'?',mtconfirmation,[mbyes,mbno],0)= mbno then exit;
        showmessage('Atenção! Já existe Alvará cadastrado para o contribuinte no exercício');
        exit;
   end;
   tbalvara.insert;
...
{        if tbalvaranumero.value = 0 then
        begin
            showmessage('Caro Usuário, favor informar o Número do Alvará');
            exit;
        end;}

        if tbalvaraexercicio.isnull = true  then
        begin
            showmessage('Caro Usuário, favor informar o Exercício Fiscal');
            exit;
        end;

      if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvaradt_duam.Value) then
      begin
         showmessage('Data do Requerimento inválida!');
         exit;
         DBEDtreq.setfocus;
      end;
 ...
 {        if tbalvaraexercicio.Value < 2022 then
   begin
      showmessage('Exercício do Requerimento Expirado!');
         exit;
         ...
         if tbalvaraexercicio.Value > 2022 then
   begin
      showmessage('Exercício do Requerimento Inválido!');
         exit;
         DBCExercicio.setfocus;
   end;
        }
...
{   if tbalvaraexercicio.Value > strtoint(formatdatetime('yyyy',date)) then
    begin
      showmessage('Impossível requerer Alvará para o exercício seguinte!');
         exit;
         DBCExercicio.setfocus;
    end;
 }
...
{        if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
        begin
                 showmessage('Usuário Não autorizado à liberação de alvará provisório!');
                 exit;
        end;    }
...
{        if tbalvaraunidade.value = ' ' then
        begin
            showmessage('Caro Usuário, favor informar a área para a qual o alvará está sendo liberado');
            exit;
        end;}
...
begin
     if messagedlg('Confirma o Cancelamento do Alvará'+#13+ tbalvaranumero.AsString + ' de '+tbalvaraexercicio.AsString +'?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
             //    if not tbalvara.Eof then tbalvara.Delete else showmessage('Não Há Alvará Solicitado para este Contribuinte!');
                   if not tbalvara.Eof then
                   begin
                      // tbalvara.Delete
                      showmessage('Informar nas observações o motivo do cancelamento');
                      tbalvara.edit;
                   ...
                   else showmessage('Não Há Alvará Solicitado para este Contribuinte!');
             end
             else showmessage('Cancelamento Não autorizado');
     end;
     btemitenovo.setfocus;
end;
...
{    DBTtaxa.Visible:=true;}
    ...
    if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
    if tbcontribhorario.Value = 'NOTURNO' then Texto_horario2.Visible := true else Texto_horario2.Visible := false;
    if tbcontribpendoc.value = true then showmessage('Atenção: Contribuinte com pendência no arquivo. Solicitar documentação. Obrigado!          SETOR DE CADASTROS.       ');
    if tbcontribpessoa.value = 'CNPJ' then
        begin
...
{    DBTtaxa.Visible:=true;}
    ...
    if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
    if tbcontribhorario.Value = 'NOTURNO' then Texto_horario2.Visible := true else Texto_horario2.Visible := false;
    if tbcontribpendoc.value = true then showmessage('Atenção: Contribuinte com pendências no arquivo. Solicitar documentação. Obrigado!          SETOR DE CADASTROS.       ');    
    if tbcontribpessoa.value = 'CNPJ' then
        begin
        DBECPF.Enabled  := false;
        DBECPF.Visible  := false;
 ...
 //         ValidarCNAESIM;
           ...
           begin
             CarregarTaxaSIM;
  //             if Assigned(FSimEndereco) then FSimEndereco.Caption := '';
             CarregarEnderecoSIM;
           end;
end;
...
begin
if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
showmessage('Atenção! Alteração do status de inatividade');
end;
...
{if filtro = 0 then
begin
        showmessage('Atenção! Exibição: somente INATIVOS');
        tbcontrib.Filter := 'inatividade = true';
        filtro := 1;
end
else
begin
        showmessage('Atenção! Exibição: somente ATIVOS');
        tbcontrib.Filter := 'inatividade <> true';
        filtro := 0;
end;}
...
begin
        if messagedlg('Confirma a Liberação Alvará o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
begin
  if (frmlogin.c_grp = 'FIS') or (frmlogin.c_grp = 'ADM')  then
    begin
       ...
       showmessage('Alvará liberado!');
    end
  else
     begin
       tbalvaralibera.Value := false;
       tbalvaraautoridade.value := ' ';
       showmessage('Função permitida somente para pessoal técnico');
     end
  end
...
begin
//   Sthora.Caption:=copy(timetostr(time),1,5);
  if not Assigned(TimerInatividade) then Exit;
  if not Visible or not Showing then Exit;
  if SecondsBetween(Now, FLastActivity) < INATIVIDADE_SEGUNDOS then Exit;

  TimerInatividade.Enabled := False;

  if tbrt.Active          and (tbrt.State in [dsEdit, dsInsert])         then tbrt.Cancel;
  if tbhistorico.Active   and (tbhistorico.State in [dsEdit, dsInsert])  then tbhistorico.Cancel;
  if tbreq.Active         and (tbreq.State in [dsEdit, dsInsert])        then tbreq.Cancel;
  if tboficio.Active      and (tboficio.State in [dsEdit, dsInsert])     then tboficio.Cancel;
  if tbtramitacao.Active  and (tbtramitacao.State in [dsEdit, dsInsert]) then tbtramitacao.Cancel;
  if tbplanta.Active      and (tbplanta.State in [dsEdit, dsInsert])     then tbplanta.Cancel;
  if tbalvlib.Active      and (tbalvlib.State in [dsEdit, dsInsert])     then tbalvlib.Cancel;
  if tbveiculo.Active and (tbveiculo.State in [dsEdit, dsInsert]) then tbveiculo.Cancel;
  if tbvisitas.Active and (tbvisitas.State in [dsEdit, dsInsert]) then tbvisitas.Cancel;
  if tbalvara.Active  and (tbalvara.State in [dsEdit, dsInsert])  then tbalvara.Cancel;
  if tbcontrib.Active and (tbcontrib.State in [dsEdit, dsInsert]) then tbcontrib.Cancel;

  if Assigned(FSimStatus) then
    FSimStatus.Caption := 'Formulario fechado por inatividade.';
...
procedure Tfrmestabe.FormMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin
  FLastActivity := Now;
end;
...
begin
{  if (tbvisitash_fim.value < tbvisitash_visita.value) and (tbvisitastipo.Value='CERTIDÃO') and (tbvisitastipo.Value='CONTRA RAZAO') and (tbvisitastipo.Value='PRORROGACAO') and (tbvisitastipo.Value='REPLICA FISCAL') and (tbvisitastipo.Value='ATO ADMINISTRATIVO DA GERÊNCIA') then
        begin
              showmessage('Atenção: Final da Inspeção Anterior ao Início');
              exit;
        end;       }
...
begin
if (tbvisitastipo.Value = 'ATO ADMINISTRATIVO DA GERÊNCIA') and (frmlogin.c_grp <> 'FIS') then
    tbvisitasfiscal1.value:= 'GERENTE DE VIGILÂNCIA SANITÁRIA';
end;
 ...
 exit;
  ...
  begin
    if (frmlogin.c_grp = 'ADM') or (frmlogin.c_grp = 'CAD') then
      begin
        if MessageDlg(
           'Atenção: os CNAEs do CVS divergem do arquivo do SIM.' + #13#10 +
           'Confirma a emissão do alvará assim mesmo?',
           mtConfirmation, [mbYes, mbNo], 0) = mrNo then
         Exit;
      end
      else
      begin
         ShowMessage('Emissão bloqueada. Os CNAEs do CVS divergem do arquivo do SIM. Verifique a aba Complementar.');
         Exit;
      end;
    end;
{if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
begin
 showmessage('Usuário Não autorizado a emitir alvará provisório!');
 exit;
end;         }
  tbrt.First;
  while not tbrt.Eof do
...
begin
 showmessage('Exercício Não requerido!');
 exit;
end;

if (tbalvlib.eof) and (tbalvlib.bof)  then
begin
 showmessage('Não existe atividade liberada para o exercício!');
 exit;
end;
...
if FaltaRt <> '' then
begin
  ShowMessage('Os CNAEs abaixo constam no alvará mas Não estão no cadastro do contribuinte. Favor ajustar:' + #13#10 + FaltaRt);
  Exit;
end;

if not tbalvaradt_emite.isnull then
begin
    showmessage('Alvará já Emitido!');
    exit;
end;
...
{if tbalvaranumero.value > 0 then
begin
    showmessage('Alvará já expedido. Escolha Reemissão!');
    exit;
end;}
  ...
  begin
    if (frmlogin.c_grp = 'ADM') or (frmlogin.c_grp = 'CAD') then
      begin
        if MessageDlg(
           'Atenção: os CNAEs do CVS divergem do arquivo do SIM.' + #13#10 +
           'Confirma a reemissão do alvará assim mesmo?',
           mtConfirmation, [mbYes, mbNo], 0) = mrNo then
         Exit;
      end
      else
      begin
         ShowMessage('Reemissão bloqueada. Os CNAEs do CVS divergem do arquivo do SIM. Verifique a aba Complementar.');
         Exit;
      end;
    end;
...
begin
    showmessage('Alvará Não Emitido!');
    exit;
end;
...
begin
  ...
  begin
      ...
      begin
          ...
          begin
             showmessage('Há pelo menos uma atividade econômica cadastrada com a subclasse Não informada. Selecione alterar e complete o cadastro antes de incluir outra atividade economía!');
             exit;
          end;
          tbauxrt.Next;
      end;
...
begin
  ...
  begin
      if messagedlg('Confirma a Exclusão da Atividade: '+#13+tbrtsubclasse.value+' - '+tbcnaeatividade.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbrt.delete;
  end
  else showmessage('Perfil Não Autorizado!');
...
begin
  ...
  if tbrtsubclasse.isnull  then
  begin
     showmessage('Favor informar o código CNAE!');
     exit;
  end;

  if (tbrttipo.Value <> 'Principal') and (tbrttipo.Value <> 'Secundária')then
  begin
     showmessage('Favor selecionar se a Atividade é a Principal ou Secundária!');
     exit;
  end;
  ...
  begin
      ...
      begin
          showmessage('Atividade já cadastrada para contribuinte!');
          exit;
      end;
      if (tbrttipo.value = 'Principal') and (tbauxrttipo.value = 'Principal') and (tbrtcontrole.value <> tbauxrtcontrole.value) then
      begin
          showmessage('Atividade Principal já cadastrada para contribuinte!');
          exit;
      end;
      tbauxrt.Next;
  end;
...
{        if tbcontribatividade.value = 0 then
    ...
    begin
            if messagedlg('Atenção!!!!! Exercício anterior. Confirma liberação para o ano PASSADO ?  ',mtconfirmation,[mbyes,mbno],0)=mrno then  exit;
...
begin
  ...
  begin
         showmessage('Caro Usuário, favor Informar a atividade a ser liberada.');
         exit;
  end;
  cae2:= false;
  tbauxrt.First;
 ...
 {     if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvlibData.Value) then
      begin
         showmessage('Exercício da liberação diferente do exercício requerido');
      end;
         }
  ...
  begin
          ...
          begin
              showmessage('Atenção! Informar a autoridade que liberou a atividade econômica');
              DBCAutoridade.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Atenção! Informar O documento de liberação da atividade econômica');
              DBCAutoridade.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Atenção! Informar a data da liberação da atividade econômica');
              DBCAutoridade.SetFocus;
              exit;
          end;
      ...
      //                 if (tbvisitaslibera.Value = 'Liberado') or (tbvisitaslibera.Value = 'Alguns CNAEs') then showmessage('ok');
                         if (tbvisitaslibera.Value = 'Não Liberado') then
                         begin
                             showmessage('Atenção! A Autoridade Sanitária relatou Não conformidade na inspeção')
                         end;
                         digitado := true;
                     end;
                 end;
          ...
          if digitado = false then showmessage ('Atenção!!! Documento de liberação da atividade econômica Não localizado. Click em Ok para continuar ...');

  end
  else showmessage('Baixa Complexidade. Liberação Automática');
...
begin
    ...
    begin
      showmessage('Atividade Econômica já liberada para o exercício!');
      exit;
    end;
    tbauxlib.next;
end;
   ...
   tbalvlib.post;
   showmessage('Autorização de Atividade Concluída!');
end;
...
begin
     if messagedlg('Confirma a Exclusão da Atividade: '+tbalvlibatividade.value+#13+' do Alvará: '+tbalvaranumero.AsString + ' de '+tbalvaraexercicio.AsString +'?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
                 if not tbalvaradt_emite.isnull then
                 begin
                     showmessage('Não é possível excluir. Alvará já emitido!');
                     exit;
                 end;
                 if not tbalvlib.Eof then tbalvlib.Delete else showmessage('Não Há Atividade Liberada para este Contribuinte!');
             end
             else showmessage('Exclusão Não autorizada');
     end;
end;
...
{      if (frmlogin.c_grp = 'CAD') or (frmlogin.c_grp = 'CON') then
      begin
        if tbveiculoautorizacao.Value = true then
          begin
            showmessage('Usuário Não autorizado à liberação veicular!');
            exit;
          end;
      end;}
        tbveiculo.post;
...
begin
     if messagedlg('Confirma a Exclusão do Alvará '+tbveiculoexercicio.AsString +' do Veículo '+ tbveiculomarca.value + ' Placa '+ tbveiculoplaca.value + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
                 if not tbveiculo.Eof then tbveiculo.Delete else showmessage('Não Há Alvará Veicular Solicitado para este Contribuinte!');
             end
             else showmessage('Exclusão Não autorizada');
     end;
     btimprime.setfocus;
...
begin
    ...
    begin
       showmessage('Alvará Veicular Não requerido!');
       exit;
    end;

    if tbveiculoautorizacao.Value <> true then
    begin
      showmessage('Alvará Veicular Não autorizado!');
      exit;
    end;
...
begin
  ...
  if (frmlogin.c_grp = 'FIS') or (frmlogin.c_grp = 'ADM')  then
  begin
      if tbveiculoautorizacao.Value = true then showmessage('Alvará Veicular Já Liberado')
      else
      begin
          if messagedlg('Confirma a Liberação do Alvará '+tbveiculoexercicio.AsString +' para o Veículo '+ tbveiculomarca.value + ' Placa '+ tbveiculoplaca.value + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
             begin
                 tbveiculo.edit;
...
begin
  ...
  begin
      ...
      begin
          if not tbalvaradt_emite.isnull then
          begin
                 showmessage('Já existe Alvará emitido para o contribuinte no exercício');
                 exit;
          end;
      end;
   ...
   begin
        showmessage('Impossível emitir alvará provisório para este exercício');
        exit;
   end;

      if messagedlg('Confirma a Liberação Provisória para o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
      begin
              tbalvara.edit;
              tbalvaratipo.Value:=true;
              tbalvara.Post;
              showmessage('Alvará Provisório Liberado');
      end;
  end
  else
  begin
...
begin
  ...
  begin
   ...
   if not tbalvaraexercicio.value = yearof(date) then
   begin
        showmessage('Impossível emitir alvará eventual para este exercício');
        exit;
   end;

      if messagedlg('Confirma a Liberação de Alvará de Evento para o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
      begin
              tbalvara.edit;
              tbalvaraevento.Value :=true;
              tbalvara.Post;
              showmessage('Alvará Eventual Liberado');
      end;
  end                                                                       
  else
  begin
...
{//   if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvaradt_duam.Value) then
   ...
   begin
      showmessage('Data do Requerimento inválida - PASSADO!');
            DBCExercicio.setfocus;
   end;
  }
   ...
   if tbalvaraexercicio.Value > strtoint(formatdatetime('yyyy',date)) then
    begin
      showmessage('Data do Requerimento inválida - FUTURO!');
         DBMDuam.setfocus;
   end;
end;
...
begin
   ...
   begin
      showmessage('Data da liberação inválida!');
...
//        showmessage('Atencao!!! Conferir o numero do processo (EM VERMELHO) que esta tramitando/consultanto');
    ...
    begin;
        showmessage('Atenção, salvar a edição antes de mudar de aba. Os dados Não salvos serão perdidos. Cancelando a operação...');
        tbtramitacao.Cancel;
        tbplanta.cancel;
        tbveiculo.Cancel;
        tbalvlib.Cancel;
...
BtFiltro.Enabled        := tbplanta.state =  dsbrowse;
...
begin
  if (tbplanta.Bof) and (tbplanta.Eof) then showmessage('Não existe protocolo pra este regulado') else
  begin
    if (frmlogin.c_grp = 'ADM') or (frmlogin.c_grp = 'CAD') or (frmlogin.c_per = 'ORD') then
    begin
        if  (tbplantacarga.Value = 'PROTOCOLO') or (tbplantacarga.Value = 'ARQUIVO') or (tbplantacarga.Value = 'OUTRA REPARTIÇÃO') or (tbplantacarga.Value = 'SERVIÇO DE NOTIFICAÇÃO') or (tbplantacarga.Value = 'TURMA JULGADORA') or (tbplantacarga.Value = 'GERÊNCIA') or (tbplantacarga.Value = 'DIRETORIA') or (tbplantacarga.Value = 'NÚCLEO DE ENGENHARIA') or (tbplantacarga.Value = 'NÚCLEO DO PAS') or (tbplantacarga.Value = 'NÚCLEO DO RPA') or (tbplantacarga.Value = frmlogin.c_user) then
        begin
            tbtramitacao.insert;
            ...
            if (frmlogin.c_grp = 'ADM')  then
            begin
                showmessage('Atenção usuário administrador do sistema, o processo selecionado está com carga para :   '+tbplantacarga.Value+'. Para tramitação forçada, utilize a função JUNTADA, informe o despacho justificando o motivo da quebra de fluxo processual no sistema e anexe-o ao processo físico, .');
               { tbtramitacao.insert;
                tbtramitacaousuario.Value  := frmlogin.c_user;
                tbtramitacaodata.value     := date;
                tbtramitacaohora.Value     := time;
            ...
            else showmessage('Somente o usuário com a carga do processo pode tramitar:   '+tbplantacarga.Value);
    end
    else
    begin
        ...
        begin
           ...
           showmessage('A carga do processo Não está com o usuário');
        end;
    end;
  end;
end;
...
begin
    ...
    begin
          ...
          begin
             showmessage('Obrigatório Escolher o Assunto');
             dbcassunto.SetFocus;
             exit;
          end;
          if tbplantacomplemento.IsNull then
          begin
              showmessage('Obrigatório Informações Complementares do Processo');
          ...
          begin
              showmessage('Obrigatório Informar o nome completo da pessoa que está dando entrada no processo');
              DBEptnome.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Obrigatório Informar um documento de identificação da pessoa que está dando entrada no processo');
              DBEdoc.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Obrigatório Informar o celular da pessoa que está dando entrada no processo');
              DBEcelular.SetFocus;
              exit;
          end;
    ...
    begin
        if tbtramitacaodescricao.IsNull then
          begin
             showmessage('Obrigatório Selecionar a Descrição da fase');
             DBCdescric.SetFocus;
             exit;
          end;
          if tbtramitacaodestino.IsNull then
          begin
             showmessage('Obrigatório Selecionar a destinação do processo');
             DBCdestino.SetFocus;
             exit;
          end;
          ...
          begin
                ...
                begin
                    showmessage('Obrigatório Informar o documento que está sendo juntado ao processo');
                    DBCJuntada.SetFocus;
                    exit;
                end;
                if tbtramitacaointeressado.IsNull then
                begin
                    showmessage('Obrigatório Informar o nome da pessoa que está juntando documento ao processo');
                    DBEInteressado.SetFocus;
                    exit;
                end;
                if (tbtramitacaojuntada.Value = 'Despacho da Gerencia') and (frmlogin.c_grp <> 'ADM') then
                begin
                  showmessage('Opção inválida para o perfil');
                  DBCJuntada.SetFocus;
                  exit;
                end;
          end;
...
begin
    showmessage('Função Não permitida para o perfil');
end;
...
BtFiltro.Enabled        := tbtramitacao.state =  dsbrowse;
...
begin
  if (tbtramitacao.Bof) and (tbtramitacao.Eof) then showmessage('Não existe tramitação pra este regulado') else
  begin
    if tbtramitacaousuario.Value <> frmlogin.c_user then showmessage('Somente o usuário que tramita pode emitir o termo de remessa') else
    begin
      if tbtramitacaojuntada.IsNull then
      begin
          painel := 'protocolo';
      ...
      else showmessage('O Registro selecionado Não é uma Tramitação. É uma juntada de documentos');
    end;
  end;
end;
...
if (tbplanta.Bof) and (tbplanta.Eof) then showmessage('Não existe protocolo pra este regulado') else
    begin
        ...
        begin
          ...
          else showmessage('Processo com carga para fiscalização. impossível efetuar juntada de documento');
        end
        else
        if (frmlogin.c_grp = 'ADM') or (frmlogin.c_per = 'JUR') then
        begin
...
begin
  ...
  begin
   if (frmlogin.c_grp = 'ADM') or  (frmlogin.c_grp = 'CAD') then
   begin
     if tbtramitacaojuntada.IsNull then showmessage('O Registro selecionado Não é uma Juntada de documentos. É uma tramitação') else
     begin
        painel := 'protocolo';
...
procedure Tfrmestabe.BtFiltroClick(Sender: TObject);
begin
  FiltroProtoIdx := (FiltroProtoIdx + 1) mod 4;
  case FiltroProtoIdx of
    0: begin
         tbplanta.Filtered := false;
         BtFiltro.Caption  := 'Todos';
       end;
    1: begin
         tbplanta.Filtered := true;
         tbplanta.Filter   := 'assunto=' + QuotedStr('REGULARIDADE DE PRODUTO ALIMENTÍCIO');
         BtFiltro.Caption  := 'RPA';
       end;
    2: begin
         tbplanta.Filtered := true;
         tbplanta.Filter   := 'assunto=' + QuotedStr('PROCESSO ADMINISTRATIVO SANITÁRIO');
         BtFiltro.Caption  := 'PAS';
       end;
    3: begin
         tbplanta.Filtered := true;
...
begin
    ...
    if tbvisitasmodalidade.value = 'DENÚNCIA'     then DBEDen.setfocus;
    if tbvisitasmodalidade.value = 'DE OFÍCIO'    then DBEOf.setfocus;
end;
...
begin
  ...
  begin
    ...
    ShowMessage('Você escolheu: ' + OpenDialog1.FileName);
...
begin
  ShowMessage('Erro CSV (Requerimento/Tramitação/Ofício/Denúncia/Planta):' + sLineBreak + Erro);
  Exit;
end;
...
begin
  ShowMessage('Erro CSV (Alvará/Liberação):' + sLineBreak + Erro);
  Exit;
end;
...
{    // Gera TRAMITACAO ENXUTA (sobrescreve tramitacao.csv com filtro + ultimo por protocolo)
    ...
    begin
      ShowMessage('Erro CSV (Tramitação Enxuta):' + sLineBreak + Erro);
      Exit;
    end;     }
...
if FSimRazao = nil then
begin
  FSimRazao := TLabel.Create(Self);
  FSimRazao.Parent := Tabident;
  FSimRazao.Left := 4;
  FSimRazao.Top := 400;
  FSimRazao.Width := 630;
  FSimRazao.Caption := '';
  FSimRazao.Font.Style := [fsBold];
  FSimRazao.Font.Color := clNavy;
end;

if FSimEndereco = nil then
begin
  FSimEndereco := TLabel.Create(Self);
  FSimEndereco.Parent := Tabident;
  FSimEndereco.Left := 4;
  FSimEndereco.Top := 420;
  FSimEndereco.Width := 630;
  FSimEndereco.Caption := '';
  FSimEndereco.Font.Style := [fsBold];
  FSimEndereco.Font.Color := clNavy;
end;
  ...
  begin
    ...
    FSimGrid.ColWidths[0] := 65;
    FSimGrid.ColWidths[1] := FSimGrid.Width - 235;
    FSimGrid.ColWidths[2] := 90;
    FSimGrid.ColWidths[3] := 70;
  end;

  if (FTSimAux = nil) and FileExists(CaminhoSIMCad) then
  begin
    FTSimAux := TTable.Create(Self);
    FTSimAux.DatabaseName := 'wcvs';
    FTSimAux.TableName    := 'SIMCAD.DB';
    FTSimAuxTemIndice := False;
    try
      FTSimAux.Open;
      FTSimAux.IndexDefs.Update;
      FTSimAuxTemIndice := (FTSimAux.IndexDefs.IndexOf('INSCRICAO') >= 0) and
                           (FTSimAux.IndexDefs.IndexOf('DOCUMENTO') >= 0);
    except
      FreeAndNil(FTSimAux);
    end;
  end;
...
function Tfrmestabe.CaminhoSIMCad: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIMCAD.DB';
end;

function Tfrmestabe.CaminhoSIMCnae: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIMCNAE.DB';
end;

function Tfrmestabe.CaminhoSIMAuxCSV: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIM\cnae_aux.csv';
end;

procedure SalvarTextoUtf8Bom(const Arquivo: string; const Texto: string);
var
  FS: TFileStream;
...
begin
  ...
  try
    ...
    FS.Write(BOM[0], 3);
    U8 := Utf8Encode(WideString(Texto));
    if Length(U8) > 0 then
      FS.Write(U8[1], Length(U8));
  finally
    FS.Free;
  end;
end;

function CsvCampoSim(const S: string): string;
begin
...
function Tfrmestabe.ExportarSIMAUXParaCNAEAuxCSV: Boolean;
var
  TCnae, TCad: TTable;
  SL: TStringList;
  Lin, Inscricao, UltimaInscricao: string;
  Razao, Documento, Endereco, Cep: string;
  TemIndiceCad: Boolean;
begin
  Result := False;
  if not FileExists(CaminhoSIMCnae) then Exit;
  if not FileExists(CaminhoSIMCad)  then Exit;

  TCnae := TTable.Create(nil);
  TCad  := TTable.Create(nil);
  SL    := TStringList.Create;
  try
    TCnae.DatabaseName := 'wcvs';
    TCnae.TableName    := 'SIMCNAE.DB';
    TCnae.Open;

    TCad.DatabaseName := 'wcvs';
    TCad.TableName    := 'SIMCAD.DB';
    TCad.Open;
    TCad.IndexDefs.Update;
    TemIndiceCad := TCad.IndexDefs.IndexOf('INSCRICAO') >= 0;

    SL.Add('INSCRICAO_ISS;COD_CONTRIBUINTE;CONTRIBUINTE;CNAE;COD_ATIVIDADE;DOCUMENTO;ATIVIDADE;SITUACAO;ATIVIDADE_ECON;ENDERECO;CEP;IND_PRINCIPAL');

    UltimaInscricao := #0;
    Razao := ''; Documento := ''; Endereco := ''; Cep := '';

    TCnae.First;
    while not TCnae.Eof do
    begin
      Inscricao := Trim(TCnae.FieldByName('INSCRICAO').AsString);

      { lookup em SIMCAD somente quando a inscricao muda — evita busca a cada linha }
      if Inscricao <> UltimaInscricao then
      begin
        Razao := ''; Documento := ''; Endereco := ''; Cep := '';
        if TemIndiceCad then
        begin
          TCad.IndexName := 'INSCRICAO';
          TCad.SetRange([Inscricao], [Inscricao]);
        end
        else
        begin
          TCad.Filter := 'INSCRICAO=' + QuotedStr(Inscricao);
          TCad.Filtered := True;
        end;
        TCad.First;
        if not TCad.Eof then
        begin
          Razao     := Trim(TCad.FieldByName('RAZAO').AsString);
          Documento := Trim(TCad.FieldByName('DOCUMENTO').AsString);
          Endereco  := Trim(TCad.FieldByName('ENDERECO').AsString);
          Cep       := Trim(TCad.FieldByName('CEP').AsString);
        end;
        if TemIndiceCad then
          TCad.CancelRange
        else
          TCad.Filtered := False;
        UltimaInscricao := Inscricao;
      end;

      Lin := CsvCampoSim(Inscricao) + ';' +
             ';' +
             CsvCampoSim(Copy(Razao, 1, 254)) + ';' +
             CsvCampoSim(Trim(TCnae.FieldByName('CNAE').AsString)) + ';' +
             ';' +
             CsvCampoSim(Documento) + ';' +
             CsvCampoSim(Copy(Trim(TCnae.FieldByName('ATIVIDADE').AsString), 1, 254)) + ';' +
             'Ativo;' +
             ';' +
             CsvCampoSim(Copy(Endereco, 1, 254)) + ';' +
             CsvCampoSim(Cep) + ';' +
             CsvCampoSim(Trim(TCnae.FieldByName('IND_PRINCIPAL').AsString));
      SL.Add(Lin);
      TCnae.Next;
    end;
    ...
    TCad.Close;
    TCnae.Close;
    SalvarTextoUtf8Bom(CaminhoSIMAuxCSV, SL.Text);
    Result := True;
  finally
    TCnae.Free;
    TCad.Free;
...
begin
  ...
  { Checar datas: se cnae.csv mais novo que SIMCAD, regenerar }
  DtBruto  := FileAge(CaminhoSIMBruto);
  DtTabela := FileAge(CaminhoSIMCad);
  if (DtBruto <> -1) and (DtTabela <> -1) and (DtBruto > DtTabela) then
    Exit;

  { Datas OK; verificar se SIMCAD nao esta vazia }
  T := TTable.Create(nil);
  try
    T.DatabaseName := 'wcvs';
    T.TableName := 'SIMCAD.DB';
    try
      T.Open;
      try
        if T.IsEmpty then
...
var
  ...
  var
    k: Integer;
    Lin, SitLin, Inscricao: string;
    PrimeiroDoBloco: Boolean;
  begin
    if not (BlocoAtivo and TemCnaeVisa and (Bloco.Count > 0)) then Exit;
    PrimeiroDoBloco := True;
    for k := 0 to Bloco.Count - 1 do
    begin
      Lin    := Bloco[k];
      SitLin := UpperCase(Trim(CampoCSV(Lin, 7)));
      if (SitLin <> 'ATIVO') and (SitLin <> 'SUSPENSO') and (SitLin <> 'BAIXADO') then
        SitLin := UpperCase(Trim(CampoCSV(Lin, 8)));
      if SitLin <> 'ATIVO' then Continue;

      Inscricao := ApenasNumeros(Trim(CampoCSV(Lin, 0)));

      // dados cadastrais gravados apenas uma vez por contribuinte
      if PrimeiroDoBloco then
      begin
        TCad.Append;
        TCad.FieldByName('INSCRICAO').AsString := Inscricao;
        TCad.FieldByName('DOCUMENTO').AsString := ApenasNumeros(Trim(CampoCSV(Lin, 3)));
        TCad.FieldByName('RAZAO').AsString     := Copy(Trim(CampoCSV(Lin, 2)), 1, 100);
        TCad.FieldByName('ENDERECO').AsString  := Copy(Trim(CampoCSV(Lin, 9)), 1, 254);
        TCad.FieldByName('CEP').AsString       := Trim(CampoCSV(Lin, 10));
        TCad.Post;
        PrimeiroDoBloco := False;
      end;

      // um registro por CNAE ativo
      TCnae.Append;
      TCnae.FieldByName('INSCRICAO').AsString      := Inscricao;
      TCnae.FieldByName('CNAE').AsString           := Trim(CampoCSV(Lin, 4));
      TCnae.FieldByName('ATIVIDADE').AsString      := Copy(Trim(CampoCSV(Lin, 6)), 1, 254);
      TCnae.FieldByName('IND_PRINCIPAL').AsString  := Trim(CampoCSV(Lin, 5));
      TCnae.Post;
...
begin
  ...
  { Fecha FTSimAux antes de recriar as tabelas }
  FreeAndNil(FTSimAux);

  TCad := TTable.Create(nil);
  TCnae := TTable.Create(nil);
  try
    { SIMCAD: dados cadastrais — um registro por contribuinte }
    TCad.DatabaseName := 'wcvs';
    TCad.TableName    := 'SIMCAD.DB';
    TCad.Exclusive    := True;
    if FileExists(CaminhoSIMCad) then
      TCad.DeleteTable;
    TCad.FieldDefs.Clear;
    TCad.FieldDefs.Add('CONTROLE',  ftAutoInc, 0,   True);
    TCad.FieldDefs.Add('INSCRICAO', ftString,  20,  False);
    TCad.FieldDefs.Add('DOCUMENTO', ftString,  20,  False);
    TCad.FieldDefs.Add('RAZAO',     ftString,  100, False);
    TCad.FieldDefs.Add('ENDERECO',  ftString,  254, False);
    TCad.FieldDefs.Add('CEP',       ftString,  10,  False);
    TCad.IndexDefs.Clear;
    TCad.IndexDefs.Add('', 'CONTROLE', [ixPrimary, ixUnique]);
    TCad.IndexDefs.Add('INSCRICAO', 'INSCRICAO', [ixCaseInsensitive]);
    TCad.IndexDefs.Add('DOCUMENTO', 'DOCUMENTO', [ixCaseInsensitive]);
    TCad.CreateTable;
    TCad.Open;

    { SIMCNAE: atividades — um registro por CNAE ativo }
    TCnae.DatabaseName := 'wcvs';
    TCnae.TableName    := 'SIMCNAE.DB';
    TCnae.Exclusive    := True;
    if FileExists(CaminhoSIMCnae) then
      TCnae.DeleteTable;
    TCnae.FieldDefs.Clear;
    TCnae.FieldDefs.Add('CONTROLE',      ftAutoInc, 0,   True);
    TCnae.FieldDefs.Add('INSCRICAO',     ftString,  20,  False);
    TCnae.FieldDefs.Add('CNAE',          ftString,  20,  False);
    TCnae.FieldDefs.Add('ATIVIDADE',     ftString,  254, False);
    TCnae.FieldDefs.Add('IND_PRINCIPAL', ftString,  1,   False);
    TCnae.IndexDefs.Clear;
    TCnae.IndexDefs.Add('', 'CONTROLE', [ixPrimary, ixUnique]);
    TCnae.IndexDefs.Add('INSCRICAO', 'INSCRICAO', [ixCaseInsensitive]);
    TCnae.CreateTable;
    TCnae.Open;
      ...
      try
        ...
        begin
          ...
          if (UpperCase(SitCSV) <> 'ATIVO') and (UpperCase(SitCSV) <> 'SUSPENSO') and (UpperCase(SitCSV) <> 'BAIXADO') then
          SitCSV := Trim(CampoCSV(L, 8));
          CNAECSV := Trim(CampoCSV(L, 4));
      ...
      // Fecha ambas as tabelas para forçar rebuild dos índices secundários pelo BDE/Paradox
      TCnae.Close;
      TCad.Close;

      { Remove SIMAUX.DB legado se ainda existir — libera espaço em disco }
      if FileExists(CaminhoSIMTabela) then
        DeleteFile(CaminhoSIMTabela);
...
var
  TSim, TCad: TTable;
  DocAtual, MunAtual, InscricaoViaDoc: string;
  AchouInscricao, AchouDocumento: Boolean;
  ListaSIMCNAE, ListaSIMTipo, ListaRT, ListaIncluir, ListaExcluir: TStringList;
  i: Integer;
  Msg: string;
begin
  if (frmlogin.c_grp <> 'ADM') and (frmlogin.c_grp <> 'CAD') and (frmlogin.c_grp <> 'FIS') then
  begin
    ShowMessage('Função exclusiva dos perfis Administrativos!');
    Exit;
  end;

  if not FileExists(CaminhoSIMCnae) then
  begin
    ShowMessage('Tabela SIMCNAE não encontrada. Execute a validação na aba Complementar primeiro.');
    Exit;
  end;
  ...
  ListaSIMTipo := TStringList.Create;
  ...
  try
    ...
    try
      ...
      TSim.TableName := 'SIMCNAE.DB';
      TSim.Open;
      TSim.IndexDefs.Update;
      ...
      if MunAtual <> '' then
      begin
        if TSim.IndexDefs.IndexOf('INSCRICAO') >= 0 then
        begin
          TSim.IndexName := 'INSCRICAO';
          TSim.SetRange([MunAtual], [MunAtual]);
        end
        else
        begin
          TSim.Filter := 'INSCRICAO=' + QuotedStr(MunAtual);
          TSim.Filtered := True;
        end;
        TSim.First;
        ...
        begin
          ...
          begin
            ...
            ListaSIMTipo.Add(Trim(TSim.FieldByName('IND_PRINCIPAL').AsString));
          end;
          TSim.Next;
        end;
        if TSim.IndexDefs.IndexOf('INSCRICAO') >= 0 then
      ...
      begin
        { SIMCNAE nao tem DOCUMENTO — busca INSCRICAO via SIMCAD primeiro }
        InscricaoViaDoc := '';
        TCad := TTable.Create(nil);
        try
          TCad.DatabaseName := 'wcvs';
          ...
          TCad.Open;
          TCad.IndexDefs.Update;
          if TCad.IndexDefs.IndexOf('DOCUMENTO') >= 0 then
          begin
            TCad.IndexName := 'DOCUMENTO';
            TCad.SetRange([DocAtual], [DocAtual]);
          end
          else
          begin
            TCad.Filter := 'DOCUMENTO=' + QuotedStr(DocAtual);
            TCad.Filtered := True;
          end;
          TCad.First;
          if not TCad.Eof then
            InscricaoViaDoc := Trim(TCad.FieldByName('INSCRICAO').AsString);
          if TCad.IndexDefs.IndexOf('DOCUMENTO') >= 0 then
            TCad.CancelRange
          else
            TCad.Filtered := False;
        ...
        if InscricaoViaDoc <> '' then
        begin
          if TSim.IndexDefs.IndexOf('INSCRICAO') >= 0 then
          begin
            TSim.IndexName := 'INSCRICAO';
            TSim.SetRange([InscricaoViaDoc], [InscricaoViaDoc]);
          end
          else
          begin
            TSim.Filter := 'INSCRICAO=' + QuotedStr(InscricaoViaDoc);
            TSim.Filtered := True;
          end;
          TSim.First;
          while not TSim.Eof do
    ...
    begin
      ShowMessage('Cadastro Não localizado no SIM. Verifique inscrição municipal e CNPJ/CPF.');
      Exit;
    end;
    ...
    for i := 0 to ListaIncluir.Count - 1 do
    begin
      { Se o CNAE a incluir é Principal, rebaixa o atual Principal para Secundária }
      if UpperCase(Trim(ListaSIMTipo[ListaSIMCNAE.IndexOf(ListaIncluir[i])])) = 'S' then
      begin
        tbrt.First;
        while not tbrt.Eof do
        begin
          if (tbrtCodigo.AsInteger = tbcontribCODIGO.AsInteger) and
             (Trim(tbrtTipo.AsString) = 'Principal') then
          begin
            tbrt.Edit;
            tbrtTipo.AsString := 'Secundária';
            tbrt.Post;
          end;
          tbrt.Next;
        end;
      ...
      tbrtTipo.AsString      := IfThen(
        UpperCase(Trim(ListaSIMTipo[ListaSIMCNAE.IndexOf(ListaIncluir[i])])) = 'S',
...
var
  ListaSIMCNAE, ListaSIMDESC, ListaSIMTipo, ListaRT: TStringList;
  TSim, TCad: TTable;
  DocAtual, MunAtual: string;
  AchouInscricao, AchouDocumento: Boolean;
  InscricaoMatch, DocumentoMatch, RazaoMatch: string;
  EnderecoSIM, CEPSIM: string;
  i: Integer;
  TemIdx: Boolean;
//  T0: Cardinal;
begin
  ...
  begin
    if Assigned(FSimStatus) then
      FSimStatus.Caption := 'Tabela SIM desatualizada. Use o botão Importa SIM".';
    Exit;
  end;
  ...
  try
    if not FileExists(CaminhoSIMCad) or not FileExists(CaminhoSIMCnae) then
    begin
      ValidarCNAESIM;
      Exit;
    end;
    ...
    TCad := TTable.Create(nil);
    try
      { SIMCNAE: fonte dos CNAEs/atividades }
      TSim.DatabaseName := 'wcvs';
      TSim.TableName := 'SIMCNAE.DB';
      TSim.Open;
      TSim.IndexDefs.Update;
      TemIdx := TSim.IndexDefs.IndexOf('INSCRICAO') >= 0;

      { SIMCAD: fonte dos dados cadastrais — lookup pontual }
      TCad.DatabaseName := 'wcvs';
      TCad.TableName := 'SIMCAD.DB';
      TCad.Open;
      TCad.IndexDefs.Update;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            { lookup unico em SIMCAD para dados cadastrais }
            if TCad.IndexDefs.IndexOf('INSCRICAO') >= 0 then
            begin
              TCad.IndexName := 'INSCRICAO';
              TCad.SetRange([InscricaoMatch], [InscricaoMatch]);
            end
            else
            begin
              TCad.Filter := 'INSCRICAO=' + QuotedStr(InscricaoMatch);
              TCad.Filtered := True;
            end;
            TCad.First;
            if not TCad.Eof then
            begin
              DocumentoMatch := Trim(TCad.FieldByName('DOCUMENTO').AsString);
              RazaoMatch     := Trim(TCad.FieldByName('RAZAO').AsString);
              EnderecoSIM    := Trim(TCad.FieldByName('ENDERECO').AsString);
              CEPSIM         := Trim(TCad.FieldByName('CEP').AsString);
            end;
            if TCad.IndexDefs.IndexOf('INSCRICAO') >= 0 then
              TCad.CancelRange
            else
        ...
        if TemIdx then TSim.CancelRange else TSim.Filtered := False;
      end;

      if (not AchouInscricao) and (DocAtual <> '') then
      begin
        { SIMCNAE nao tem indice DOCUMENTO; busca SIMCAD pelo DOCUMENTO para obter a INSCRICAO }
        if TCad.IndexDefs.IndexOf('DOCUMENTO') >= 0 then
        begin
          TCad.IndexName := 'DOCUMENTO';
          TCad.SetRange([DocAtual], [DocAtual]);
        ...
        begin
          AchouDocumento := True;
          InscricaoMatch := Trim(TCad.FieldByName('INSCRICAO').AsString);
          DocumentoMatch := Trim(TCad.FieldByName('DOCUMENTO').AsString);
          RazaoMatch     := Trim(TCad.FieldByName('RAZAO').AsString);
          EnderecoSIM    := Trim(TCad.FieldByName('ENDERECO').AsString);
          CEPSIM         := Trim(TCad.FieldByName('CEP').AsString);
          { agora busca os CNAEs em SIMCNAE pela INSCRICAO encontrada }
          if TCad.IndexDefs.IndexOf('DOCUMENTO') >= 0 then
            TCad.CancelRange
          else
            TCad.Filtered := False;
          if TemIdx and (InscricaoMatch <> '') then
          begin
            TSim.IndexName := 'INSCRICAO';
            TSim.SetRange([InscricaoMatch], [InscricaoMatch]);
          end
          else if InscricaoMatch <> '' then
          begin
            TSim.Filter := 'INSCRICAO=' + QuotedStr(InscricaoMatch);
            TSim.Filtered := True;
          end;
          TSim.First;
          while not TSim.Eof do
...
//      ShowMessage('SIMCNAE+SIMCAD levou: ' + IntToStr(GetTickCount - T0) + ' ms');
    ...
    begin
      AtualizarStatusSincronizacaoSIM;
      FSimStatus.Caption := FSimStatus.Caption + ' | Cadastro Não localizado';
      FSimGrid.RowCount := 1;
      FSimDivergente := True;
      Application.ProcessMessages;
      Exit;
    ...
    begin
      ...
      begin
        ...
        FSimGrid.Cells[2, FSimGrid.RowCount - 1] := 'NÃO COMPETE';
      if i < ListaSIMTipo.Count then
        FSimGrid.Cells[3, FSimGrid.RowCount - 1] :=
          IfThen(UpperCase(Trim(ListaSIMTipo[i])) = 'S', 'Principal', 'Secundária')
      else
        FSimGrid.Cells[3, FSimGrid.RowCount - 1] := '';
    end;
    ...
    if Assigned(FSimEndereco) then
    begin
      if EnderecoSIM <> '' then
        FSimEndereco.Caption := 'Endereço: ' + EnderecoSIM + ' - ' + CEPSIM
      else
        FSimEndereco.Caption := '';
    end;
...
begin
  Result := '';
  if not tbalvlib.Active then Exit;
  if not tbrt.Active then Exit;

  Salvo := tbalvlib.GetBookmark;
  tbalvlib.DisableControls;
  try
    tbalvlib.First;
    while not tbalvlib.Eof do
    begin
      Atividade := Trim(tbalvlibAtividade.AsString);
      if Atividade <> '' then
      begin
        SalvoRT := tbrt.GetBookmark;
        tbrt.DisableControls;
        try
          Encontrado := tbrt.Locate('Subclasse', Atividade, [loCaseInsensitive]);
        finally
          tbrt.GotoBookmark(SalvoRT);
          tbrt.FreeBookmark(SalvoRT);
          tbrt.EnableControls;
        end;
        if not Encontrado then
        begin
          if Result <> '' then Result := Result + #13#10;
          Result := Result + '- ' + Atividade;
        end;
      end;
      tbalvlib.Next;
    end;
  finally
    tbalvlib.GotoBookmark(Salvo);
    tbalvlib.FreeBookmark(Salvo);
    tbalvlib.EnableControls;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      DataAux := FileDateToDateTime(DtAux);
      FSimStatus.Caption :=
        'API SIM: bruto ' + FormatDateTime('dd/mm/yyyy hh:nn', DataAux) + ' (tabela Não gerada)';
      Exit;
    end;
  end;

  FSimStatus.Caption := 'API SIM: Não disponivel';
end;
...
begin
  ...
  begin
    try
      if FileExists(CaminhoSIMCad) and FileExists(CaminhoSIMCnae) and not PrecisaGerarSIMTabela then
      begin
        ValidarCNAESIM_Tabela;
        Exit;
      end;
  ...
  begin
    if PrecisaGerarSIMTabela then
      ...
      FSimStatus.Caption := 'Tabela SIM Não disponível. Use o botão Importa SIM".';
  end;
end;

{ ============================================================
  Importacao de taxa.csv (Taxa de Vigilancia Sanitaria - SIM)
  ============================================================ }

function Tfrmestabe.CaminhoTaxaCSV: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIM\taxa.csv';
end;

function Tfrmestabe.CaminhoTaxaTabela: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIMTAXA.DB';
end;

function Tfrmestabe.ImportarTaxaSIM: Boolean;
var
  ArqIn: TextFile;
  L, LProx: string;
  InscricaoStr, SituacaoStr, ObsStr, ComplexStr: string;
  ValorStr: string;
  ExercicioVal: Integer;
  ValorVal, AreaVal: Double;
  VencDate: TDateTime;
  PagoVal: Boolean;
  T: TTable;
  RegCount: Integer;

  function ExtrairComplexidade(const Obs: string): string;
  begin
    if Pos('Alta', Obs) > 0 then
      Result := 'Alta'
    else if Pos('dia', Obs) > 0 then
      Result := 'M' + Chr(233) + 'dia'
    else if Pos('Baixa', Obs) > 0 then
      Result := 'Baixa'
    else
      Result := '';
  end;

  function ExtrairArea(const LinhaArea: string): Double;
  var
    S: string;
    P: Integer;
  begin
    ...
    S := Trim(LinhaArea);
    P := Pos('* ' + Chr(193) + 'rea:', S);
    if P = 0 then P := Pos('* Area:', S);
    if P > 0 then
      S := Trim(Copy(S, P + 8, Length(S)))
    else
      Exit;
    P := Pos('m', S);
    if P > 0 then
  ...
  { Parse dd/mm/aaaa sem TFormatSettings (compativel com Delphi 7) }
  function ParseData(const S: string): TDateTime;
  var
    d, m, y: Word;
    Sv: string;
  begin
    Result := 0;
    Sv := Trim(S);
    if Length(Sv) < 10 then Exit;
    d := StrToIntDef(Copy(Sv, 1, 2), 0);
    m := StrToIntDef(Copy(Sv, 4, 2), 0);
    y := StrToIntDef(Copy(Sv, 7, 4), 0);
    if (d = 0) or (m = 0) or (y = 0) then Exit;
    try
...
begin
  Result := False;
  if not FileExists(CaminhoTaxaCSV) then
  begin
    ShowMessage('Arquivo taxa.csv n' + Chr(227) + 'o encontrado:' + #13#10 + CaminhoTaxaCSV);
    Exit;
  end;
  ...
  try
    T.DatabaseName := 'wcvs';
    T.TableName    := 'SIMTAXA.DB';
    T.Exclusive    := True;

    if FileExists(CaminhoTaxaTabela) then
      T.DeleteTable;

    T.FieldDefs.Clear;
    T.FieldDefs.Add('CONTROLE',     ftAutoInc,  0,  True);
    T.FieldDefs.Add('INSCRICAO',    ftString,   20, False);
    T.FieldDefs.Add('EXERCICIO',    ftSmallInt, 0,  False);
    T.FieldDefs.Add('VALOR',        ftFloat,    0,  False);
    T.FieldDefs.Add('DT_VENC',      ftDate,     0,  False);
    T.FieldDefs.Add('SITUACAO',     ftString,   30, False);
    T.FieldDefs.Add('PAGO',         ftBoolean,  0,  False);
    T.FieldDefs.Add('COMPLEXIDADE', ftString,   10, False);
    T.FieldDefs.Add('AREA',         ftFloat,    0,  False);
    T.IndexDefs.Clear;
    T.IndexDefs.Add('', 'CONTROLE', [ixPrimary, ixUnique]);
    T.IndexDefs.Add('INSCRICAO', 'INSCRICAO', [ixCaseInsensitive]);
    T.CreateTable;
    T.Open;

    RegCount := 0;
    AssignFile(ArqIn, CaminhoTaxaCSV);
    Reset(ArqIn);
    try
      if not EOF(ArqIn) then ReadLn(ArqIn, L); { pular cabecalho }
      ...
      begin
        ...
        InscricaoStr := Trim(CampoCSV(L, 4));
        if InscricaoStr = '' then Continue;

        ExercicioVal := StrToIntDef(Trim(CampoCSV(L, 6)), 0);

        ValorStr := Trim(CampoCSV(L, 8));
        ValorStr := StringReplace(ValorStr, '.', '', [rfReplaceAll]); { remove separador de milhar }
        ValorVal := StrToFloatDef(ValorStr, 0); { DecimalSeparator do sistema = ',' (pt-BR) }
        
        VencDate    := ParseData(Trim(CampoCSV(L, 9)));
        SituacaoStr := Trim(CampoCSV(L, 10));
        PagoVal     := UpperCase(Trim(SituacaoStr)) = 'PAGO';
        ObsStr      := Trim(CampoCSV(L, 11));
        ComplexStr  := ExtrairComplexidade(ObsStr);
        ...
        begin
          ReadLn(ArqIn, LProx);
          LProx := Trim(LProx);
          if Copy(LProx, 1, 1) = '*' then
            AreaVal := ExtrairArea(LProx);
        end;

        T.Append;
        T.FieldByName('INSCRICAO').AsString    := ApenasNumeros(InscricaoStr);
        T.FieldByName('EXERCICIO').AsInteger   := ExercicioVal;
        T.FieldByName('VALOR').AsFloat         := ValorVal;
        if VencDate > 0 then
          T.FieldByName('DT_VENC').AsDateTime  := VencDate;
        T.FieldByName('SITUACAO').AsString     := Copy(SituacaoStr, 1, 30);
        T.FieldByName('PAGO').AsBoolean        := PagoVal;
        T.FieldByName('COMPLEXIDADE').AsString := ComplexStr;
        T.FieldByName('AREA').AsFloat          := AreaVal;
        T.Post;

        Inc(RegCount);
        if RegCount mod 1000 = 0 then Application.ProcessMessages;
      end;
    finally
      CloseFile(ArqIn);
    end;
    ...
    ShowMessage('Importação concluída: ' +
      IntToStr(RegCount) + ' registros gravados em SIMTAXA.DB.');
  except
    on E: Exception do
    begin
      ShowMessage('Erro ao importar taxa.csv:' + #13#10 + E.Message);
      Result := False;
    end;
  end;
  T.Free;
...
function Tfrmestabe.TabelaEmUso(const NomeTabela: string): Boolean;
var
  T: TTable;
begin
  Result := False;
  if not FileExists(IncludeTrailingPathDelimiter(AliasPath('WCVS')) + NomeTabela) then
    Exit;
  T := TTable.Create(nil);
  try
    T.DatabaseName := 'wcvs';
    T.TableName    := NomeTabela;
    T.Exclusive    := True;
    try
      T.Open;
      T.Close;
...
procedure Tfrmestabe.BtimportaClick(Sender: TObject);
var
  TabelasEmUso: string;
  TaxaEmUso, SIMCADEmUso, SIMCNAEEmUso: Boolean;
begin
  if (frmlogin.c_grp <> 'ADM') and (frmlogin.c_grp <> 'CAD') then
  begin
    ShowMessage('Fun' + Chr(231) + Chr(227) + 'o exclusiva dos perfis Administrativos!');
    Exit;
  end;
  ...
  if not FileExists(CaminhoSIMBruto) then
  begin
    ShowMessage('Arquivo cnae.csv n' + Chr(227) + 'o encontrado:' + #13#10 + CaminhoSIMBruto);
    Exit;
  end;

  { Fechar nossa propria referencia antes de testar uso exclusivo }
  if tbtaxa.Active then tbtaxa.Close;
  FreeAndNil(FTSimAux);

  TaxaEmUso    := TabelaEmUso('SIMTAXA.DB');
  SIMCADEmUso  := TabelaEmUso('SIMCAD.DB');
  SIMCNAEEmUso := TabelaEmUso('SIMCNAE.DB');

  if TaxaEmUso or SIMCADEmUso or SIMCNAEEmUso then
  begin
    TabelasEmUso := '';
    if TaxaEmUso    then TabelasEmUso := TabelasEmUso + '  - SIMTAXA.DB'  + #13#10;
    if SIMCADEmUso  then TabelasEmUso := TabelasEmUso + '  - SIMCAD.DB'   + #13#10;
    if SIMCNAEEmUso then TabelasEmUso := TabelasEmUso + '  - SIMCNAE.DB'  + #13#10;
    ShowMessage(
      'N' + Chr(227) + 'o foi poss' + Chr(237) + 'vel iniciar a importa' + Chr(231) + Chr(227) + 'o.' + #13#10 +
      'As seguintes tabelas est' + Chr(227) + 'o em uso por outro usu' + Chr(225) + 'rio:' + #13#10 +
      TabelasEmUso + #13#10 +
      'Feche todas as inst' + Chr(226) + 'ncias do sistema antes de continuar.'
    );
    if not tbtaxa.Active then tbtaxa.Open;
    Exit;
  end;

  if MessageDlg(
    'Isso vai recriar as tabelas SIMTAXA.DB, SIMCAD.DB e SIMCNAE.DB.' + #13#10 +
    'Todos os registros anteriores ser' + Chr(227) + 'o apagados.' + #13#10#13#10 +
    'Deseja continuar?',
  ...
  frmprincipal.tbsequencia.Edit;
  frmprincipal.tbsequenciadisponibilidade.Value := False;
  frmprincipal.tbsequencia.Post;

  try
    ImportarTaxaSIM;
    GerarSIMTabela;
    ExportarSIMAUXParaCNAEAuxCSV;
  finally
    if not tbtaxa.Active then tbtaxa.Open;
    CarregarTaxaSIM;
    frmprincipal.tbsequencia.Edit;
    frmprincipal.tbsequenciadisponibilidade.Value := True;
    frmprincipal.tbsequencia.Post;
  end;
end;

procedure Tfrmestabe.AtualizarLbTimestamp;
var
  DtArq: Integer;
begin
  if not Assigned(LbTimestamp) then Exit;
  DtArq := FileAge(CaminhoTaxaTabela);
  if DtArq = -1 then
    LbTimestamp.Caption := '(n' + Chr(227) + 'o gerado)'
  else
    LbTimestamp.Caption := FormatDateTime('dd/mm/yyyy hh:nn', FileDateToDateTime(DtArq));
end;

procedure Tfrmestabe.CarregarEnderecoSIM;
var
  MunAtual, DocAtual, ChaveAtual: string;
  Achou: Boolean;
  RazaoAux: string;
begin
  InicializarPainelSIM;
  ...
  ChaveAtual := MunAtual + '|' + DocAtual;

  if (MunAtual = '') and (DocAtual = '') then
  begin
    FLastSIMChave    := '';
    FLastSIMEndereco := '';
    FLastSIMRazao    := '';
  ...
  if ChaveAtual = FLastSIMChave then
  begin
    FSimEndereco.Caption := FLastSIMEndereco;
    FSimRazao.Caption    := FLastSIMRazao;
    Exit;
  end;

  FLastSIMChave    := ChaveAtual;
  FLastSIMEndereco := '';
  FLastSIMRazao    := '';

  if (FTSimAux = nil) or not FTSimAux.Active then Exit;

  Achou    := False;
  RazaoAux := '';
  try
    FTSimAux.DisableControls;
    try
      if MunAtual <> '' then
      begin
        if FTSimAuxTemIndice then
        begin
          FTSimAux.IndexName := 'INSCRICAO';
          FTSimAux.SetRange([MunAtual], [MunAtual]);
        end
        else
        begin
          FTSimAux.Filter   := 'INSCRICAO=' + QuotedStr(MunAtual);
          FTSimAux.Filtered := True;
        end;
        FTSimAux.First;
        if not FTSimAux.Eof then
        begin
          RazaoAux         := Trim(FTSimAux.FieldByName('RAZAO').AsString);
          FLastSIMEndereco := 'Endereço: ' +
            Trim(FTSimAux.FieldByName('ENDERECO').AsString) + ' - ' +
            Trim(FTSimAux.FieldByName('CEP').AsString);
          Achou := True;
        end;
        if FTSimAuxTemIndice then
          FTSimAux.CancelRange
        else
          FTSimAux.Filtered := False;
      end;

      if (not Achou) and (DocAtual <> '') then
      begin
        if FTSimAuxTemIndice then
        begin
          FTSimAux.IndexName := 'DOCUMENTO';
          FTSimAux.SetRange([DocAtual], [DocAtual]);
        end
        else
        begin
          FTSimAux.Filter   := 'DOCUMENTO=' + QuotedStr(DocAtual);
          FTSimAux.Filtered := True;
        end;
        FTSimAux.First;
        if not FTSimAux.Eof then
  ...
  if RazaoAux <> '' then
    FLastSIMRazao := 'Razão Social: ' + RazaoAux;

  FSimEndereco.Caption := FLastSIMEndereco;
  FSimRazao.Caption    := FLastSIMRazao;
end;

procedure Tfrmestabe.CarregarTaxaSIM;
var
  InscNorm: string;
  TemIndice: Boolean;
begin
  AtualizarLbTimestamp;
  if not tbtaxa.Active then Exit;

  InscNorm := IntToStr(StrToIntDef(ApenasNumeros(tbcontribMUNICIPAL.AsString), 0));
  if InscNorm = '0' then InscNorm := '';

  TemIndice := False;
  try
    tbtaxa.IndexDefs.Update;
    TemIndice := tbtaxa.IndexDefs.IndexOf('INSCRICAO') >= 0;
  except
  end;

  tbtaxa.DisableControls;
  try
    if TemIndice then
    begin
      tbtaxa.Filtered := False;
      tbtaxa.IndexName := 'INSCRICAO';
      if InscNorm <> '' then
        tbtaxa.SetRange([InscNorm], [InscNorm])
      else
        tbtaxa.CancelRange;
    end
    else
    begin
      tbtaxa.Filtered := False;
      if InscNorm <> '' then
        tbtaxa.Filter := 'INSCRICAO=' + QuotedStr(InscNorm)
      else
        tbtaxa.Filter := 'INSCRICAO=''XXXXXXXXXXXXXXXXXX''';
      tbtaxa.Filtered := True;
      tbtaxa.First;
    end;
  finally
    tbtaxa.EnableControls;
  end;
end;
