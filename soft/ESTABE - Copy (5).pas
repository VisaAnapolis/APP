...
{    filtro:integer;}
    ...
    function CaminhoSIMAux: string;
    function CaminhoSIMLock: string;
    ...
    function PrecisaGerarAuxSIM: Boolean;
    function GerarAuxSIM: Boolean;
...
{       tbvisitas.insert;
   ...
   else showmessage('Funùùo permitida somente para pessoal administrativo');
...
if frmlogin.c_user = 'CLùUDIO NASCIMENTO SILVA' then
begin
    btapi.Enabled := true;
end;
...
{    if (tbsinavisacomplexidade.value <> complex) and (complex <> '' ) and (frmlogin.c_grp <> 'ADM') then
         begin
            showmessage('Alteraùùo da Complexidade do Contribuinte nùo autorizada para o usuùrio.');
            DBLGrupo.setfocus;
            exit;
      end;}
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' jù cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Usuùrio nùo autorizado',mtwarning,[mbok],0);
//            if messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' jù cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicaùùo do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
           exit;
          end
          else if messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' jù cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicaùùo do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
       end;
    end;
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CPF: '+tbauxcontribcpf.asstring+' jù cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Perfil nùo autorizado',mtwarning,[mbok],0);
          exit;
         end
         else if messagedlg('CPF: '+tbauxcontribcpf.asstring+' jù cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicaùùo do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
       end;
    end;
        ...
        begin
            showmessage('Caro Usuùrio, favor informar a Razùo Social');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuùrio, favor informar o tipo de Cadastro');
            exit;
        end;
        ...
        if tbcontribpessoa.value = 'CNPJ' then
            begin
                ...
                begin
                    showmessage('Caro Usuùrio, favor informar o CNPJ');
                    exit;
                end;
                tbcontribcpf.value := '';
                If NOT CGC(tbcontribcgc.value) then
                Begin
                    messagebox(Application.Handle, Pchar ('O CNPJ ' +tbcontribcgc.value+ ' ù invùlido!'), 'Atenùùo', MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
                    exit;
                end;
            ...
            begin
                ...
                begin
                    showmessage('Caro Usuùrio, favor informar o CPF');
                    exit;
                end;
                tbcontribcgc.value := '';
                  If NOT CPF(tbcontribCPF.value) then
                  Begin
                    messagebox(Application.Handle, Pchar ('O CPF ' +tbcontribcpf.value+ ' ù invùlido!'), 'Atenùùo', MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
                    exit;
                  end;
            end;
        ...
        begin
            showmessage('Caro Usuùrio, favor informar o Logradouro');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuùrio, favor informar o Endereùo Completo');
            exit;
        end;

        if tbcontribhorario.value = '' then
        begin
            showmessage('Caro Usuùrio, favor informar o Perùodo de Funciomento');
            exit;
        end;
        ...
        begin
            showmessage('Caro Usuùrio, favor Selecionar um Bairro');
            exit;
        end;

{        if tbcontribhorario.IsNull then
        begin
            showmessage('Caro Usuùrio, favor informar o horùrio de funcionamento');
            exit;
        end;}
...

{
        if (tbcontribarea.value <> tbgrupoarea.value) and (tbcontribinatividade.Value <> true) then
          begin
           showmessage('Caro Usuùrio, Grupo Economico selecionado divergente da ùrea de Fiscalizaùùo');
           exit;
        end;
        if (tbcontribgrupo.value <> tbativigrupo.value) and (tbcontribinatividade.Value <> true) then
          begin
           showmessage('Caro Usuùrio, Atividade Econùmica selecionado divergente do Grupo Econùmico');
           exit;
        end;

        if (tbcontribarea.value = 0) and (tbcontribinatividade.Value <> true) then
        begin
            showmessage('Caro Usuùrio, favor Selecionar a ùrea de Fiscalizaùùo relacionada');
            exit;
        end;

        if (tbcontribgrupo.value = 0) and (tbcontribinatividade.Value <> true) then
        begin
            showmessage('Caro Usuùrio, favor Selecionar o Grupo Econùmico relacionado');
            exit;
        end;}

{        if tbcontribatividade.value = 0 then
        begin
            showmessage('Caro Usuùrio, favor Selecionar a Atividade Econùmica relacionada');
            exit;
        end;    }  
...
{  vasio:= false;
  ...
  begin
      ...
      if vasio = true then
      begin
          showmessage('Caro Usuùrio, favor completar a ficha CAE para todas atividades cadastradas');
          exit;
      end;
      tbrt.Next;
  end;
        ...
        begin
            showmessage('Caro Usuùrio, favor Preenchear a ficha CAE');
            exit;
        end;}
...
{        end;
        ...
        begin
               if not (tbrt.eof) and not (tbcaracter.eof) then
                begin
                     showmessage('Atenùùo! Para salvar as Alteraùùoes nas "ESPECIFICAùùES", marque a opùùo "RT/CARACTERIZAùùO".');
                     exit;
                end;
        end;
...
begin
  messagedlg('Mùdulo em desenvolvimento',mtwarning,[mbok],0);
{    btlocpro.setfocus;}
end;
...
case key of
        ...
        begin
            if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F3 nao suportada em modo ediùùo');
        end;
        vk_F2:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F2 nao suportada em modo ediùùo');
        end;
        vk_F1:
        begin
         if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F1 nao suportada em modo ediùùo');
        end;
        vk_F4:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F4 nao suportada em modo ediùùo');
        end;
        vk_F5:
        begin
        if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                    PageControl1.ActivePage := TabAlvara
                      ...
                      showmessage('F5 nao suportada em modo ediùùo');
        end;
...
begin
if (tbcontrib.state=dsbrowse) and  (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbveiculo.state=dsbrowse) and (tbplanta.state=dsbrowse) then statusbar1.panels[0].text:=' [F1] Ajuda   [F2] Pesquisa   [F3] Inclusùo   [F4] Alteraùùo   [F5] Alvarù' else statusbar1.panels[0].text:=' [F6] Grava   [ESC] Cancela';
...
//        tbvisitaslibera.Value := 'Nùo Liberado';
        DbCordem.setfocus;
{        tbvisitas.last;
...
else showmessage ('Perfil nùo autorizado');
...
begin
     ...
     if messagedlg('Confirma a Exclusùo do Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
...
//                     if not tbcaracter.eof      then tbcaracter.delete;
             ...
             else showmessage('Exclusùo: Favor Encaminhar Para o Setor de Cadastro');
     end;
{     btlocpro.setfocus;}
end;
...
retorno := false;
      perform(Wm_nextdlgctl,0,0);
        ...
        begin
                showmessage('Caro Usuùrio, favor informar Aùùo Fiscal');
                exit;
        end
        else
        begin
                if  tbvisitasACAO.Value= 'ATENDIMENTO ù DENùNCIA' then
                begin
                        if tbvisitasDenuncia.value = 0 then
                        begin
                                showmessage('Caro Usuùrio, favor informar Nùmero da Denùncia');
                                exit;
                        end;
                end;
        end;
        ...
        begin
            showmessage('Caro Usuùrio, favor informar qual documento Emitido');
            exit;
        end;
        if tbvisitasdt_visita.isnull then
        begin
            showmessage('Caro Usuùrio, favor informar a data do Documento' );
            DBEDataV.setfocus;
            exit;
        end;
        if (tbvisitasatividade.IsNull) and (frmlogin.c_grp = 'FIS') then
        begin
            showmessage('Caro Usuùrio, favor informar a atividade inspecionada' );
            DBEDataV.setfocus;
            exit;
        end;
        

        if (tbvisitastipo.Value = 'TERMO DE INTIMAùùO') or (tbvisitastipo.Value = 'TERMO DE NOTIFICAùùO') or (tbvisitastipo.Value = 'CERTIDùO') then
        begin
            if (tbvisitaslibera.value <> 'Sim') and (tbvisitaslibera.value <> 'Nùo') then
            begin
                showmessage('Caro Usuùrio, favor informar se a OS necessita retorno' );
                DBCLibera.setfocus;
                exit;
            end;
        end;
        ...
        begin
            ...
            begin
                showmessage('Caro Usuùrio, favor informar a origem da OS' );
                DBCOrdem.setfocus;
                exit;
            end;
        end;
       ...
       if (tbvisitasmodalidade.value <> 'REQUERIMENTO') and (tbvisitasmodalidade.value <> 'DENùNCIA') and (tbvisitasmodalidade.value <> 'DE OFùCIO') and (tbvisitasmodalidade.value <> 'PROTOCOLO') then
       begin
           if (frmlogin.c_grp = 'ADM') or (frmlogin.c_per = 'JUR') then
           else
           begin
                showmessage('Caro Usuùrio, favor selecionar uma das opùùes na origem da OS' );
                DBCOrdem.setfocus;
                exit;
           end;
       end;

        if tbvisitasmodalidade.value = 'DE OFùCIO' then
        begin
            tbvisitasos.Clear;
        ...
        if tbvisitasmodalidade.value = 'DENùNCIA' then
        begin
            tbvisitasoficio.Clear;
            tbvisitasos.clear;
            tbvisitasprotocolo.Clear;
        ...
        begin
        showmessage('Caro Usuùrio, favor informar o nùmero da OS de requerimento' );
                DBEOs.setfocus;
                exit;
        end;

        if (tbvisitasmodalidade.value = 'DENùNCIA') and (tbvisitasdenuncia.IsNull) then
        begin
                showmessage('Caro Usuùrio, favor informar o nùmero da denùncia' );
                DBEDen.setfocus;
                exit;
        end;
        if (tbvisitasmodalidade.value = 'PROTOCOLO') and (tbvisitasprotocolo.IsNull) then
        begin
                showmessage('Caro Usuùrio, favor informar o nùmero do Protocolo' );
                DBEProto.setfocus;
                exit;
        end;
        if (tbvisitasmodalidade.value = 'DE OFùCIO') and (tbvisitasoficio.IsNull) then
        begin
                showmessage('Caro Usuùrio, favor informar o nùmero da OS De Ofùcio.' );
                DBEOf.setfocus;
                exit;
        end;
...
//===============================================================================================
 { cae:= false;
  ...
  if cae = false then
  begin
      showmessage('Atividade Econùmica inexistente na ficha CAE do contribuinte!');
      exit;
  end;
   }
//===============================================================================================
        ...
        if (tbrmpfnome.value=tbvisitasfiscal1.value) or (tbrmpfnome.value=tbvisitasfiscal2.value) or (tbrmpfnome.value=tbvisitasfiscal3.value) then
           begin
               if (tbvisitasdt_visita.value=tbrmpfdata.value) then
                  begin
                     messagedlg('Relatùrio de Produùùo Homologado: '+datetostr(tbrmpfdata.value)+' - '+tbrmpfnome.value+'.',mtwarning,[mbok],0);
                     exit;
                  end;
           end;
        tbrmpf.Next;
   ...
   begin
       if dayofweek(date) = 2 then
        begin
         ...
         begin
            showmessage('Impossùvel alterar a inspeùùo de '+ datetostr(datavisita) +' para '+datetostr(tbvisitasdt_visita.value));
            DBEDataV.setfocus;
            exit;
         end;
       end
       ...
       begin
        ...
        if (tbvisitasnumero.isnull) and (tbvisitastipo.value <> 'CERTIDùO')  then
        begin
            showmessage('Caro Usuùrio, favor informar o Nùmero do Documento');
            exit;
        end;

        if (tbvisitasfiscal1.isnull) and (tbvisitasfiscal2.isnull) and (tbvisitasfiscal3.isnull) then
        begin
            showmessage('Caro Usuùrio, favor informar Pelo menos uma Autoridade Emitente');
            exit;
        end;
...
{        if not (tbvisitasfiscal3.IsNull) and not (tbvisitasfiscal2.isnull) and not (tbvisitasfiscal3.isnull) and not tbvisitasporte = 'GRANDE' then
        begin
            showmessage('Nùo permitido 3(trùs) autoridades por inspeùùo!');
            exit;
        end;
 }
        if tbvisitastipo.value = 'ATO ADMINISTRATIVO DA GERùNCIA' then
        begin
       ...
       if (tbvisitastipo.value <> 'ATO ADMINISTRATIVO DA GERùNCIA') and (frmlogin.c_grp = 'CAD' ) then
       begin
             if messagedlg('Atenùùo!!'+#13+tbvisitastipo.Value+' ù documento fiscal. Confirma a inclusùo da peùa emitida pelo fiscal '+tbvisitasfiscal1.Value+' em carùter excepecional, objetivando a atualizaùùo cadastral ?',mtconfirmation,[mbyes,mbno],0)=mrno then exit;
       end;


//**************************
{if (tbvisitastipo.Value<>'RELATùRIO TùCNICO') and (tbvisitastipo.Value<>'CERTIDùO') and (tbvisitastipo.Value<>'CONTRA RAZùO') and (tbvisitastipo.Value<>'PRORROGAùùO') and (tbvisitastipo.Value<>'RùPLICA FISCAL') and (tbvisitastipo.Value<>'ATO ADMINISTRATIVO DA GERùNCIA') then
begin

       if tbvisitash_fim.value < tbvisitash_visita.value  then
        begin
              showmessage('Final da Inpeùùo Anterior ao inùcio');
              exit;
        end;

       if tbvisitash_fim.value = tbvisitash_visita.value  then
        begin
              showmessage('Final da Inspeùùo igual ao inùcio');
              exit;
        end;

       if secondsbetween(tbvisitash_visita.value,0) < 25200 then
        begin
              showmessage('Inùcio da Inspeùùo anterior ao inùcio do expediente');
              exit;
        end;

      if secondsbetween(tbvisitash_visita.value,0) > 79200 then
        begin
              showmessage('Inùcio da Inspeùùo posterior ao final do expediente');
              exit;
        end;

       if secondsbetween(tbvisitash_fim.value,0) > 79200 then
        begin
              showmessage('Final da Inspeùùo posterior ao final do expediente');
              exit;
        end;
}

 if (tbvisitastipo.value = 'AUTO DE IMPOSIùùO DE PENALIDADE') or
    (tbvisitastipo.value = 'AUTO DE INFRAùùO')                or
    (tbvisitastipo.value = 'TERMO DE INTIMAùùO')              or
    (tbvisitastipo.value = 'TERMO DE NOTIFICAùùO')
 then
 begin

{       if tbvisitash_visita.isnull  then
        begin
              showmessage('Favor informar hora do inùcio da inspeùùo');
              exit;
        end;

         if tbvisitash_fim.isnull  then
        begin
              showmessage('Favor informar hora do final da inspeùùo');
              exit;
        end;
 end;
}
 ...
 {  if (tbvisitash_fim.value > time) and (tbvisitasdt_visita.value = date) then
   begin
      showmessage('Hora do fim da visita invùlida!');
      exit;
   end;
}
{   if (tbvisitasporte.isnull) and (tbvisitasacao.value='INSPEùùO SANITùRIA') then
   begin
        showmessage('Favor informar o porte da inspeùùo');
        DBCporte.setfocus;
        exit;
   end;    }
   ...
   begin
        showmessage('Data da Inspeùùo invùlida!');
        DBEDataV.setfocus;
        exit;
   end;
...
//     if (daysbetween(date,tbvisitasdt_visita.value) > 9) and (frmlogin.c_grp = 'FIS') then
     ...
     begin
        showmessage('Data retroativa para inclusùo/alteraùùo de insepeùùes!');
        DBEDataV.setfocus;
        exit;
     end;
   end
...
//**********************************************************
        ...
        begin
                if (tbvisitastipo.Value = 'TERMO DE NOTIFICAùùO')            or
                   (tbvisitastipo.Value = 'TERMO DE INTIMAùùO')              or
                   (tbvisitastipo.Value = 'TERMO DE COLETA DE AMOSTRA')      or
                   (tbvisitastipo.Value = 'CERTIDùO')                        or
                   (tbvisitastipo.Value = 'AUTO DE INFRAùùO')                or
                   (tbvisitastipo.Value = 'AUTO DE IMPOSIùùO DE PENALIDADE') then
               begin
                    showmessage('Caro Usuùrio, obrigatùrio informar meio de locomoùao para a atividade informada' );
                    DBCMeio.SetFocus;
                    exit;
               end;
        end;

        if (tbvisitasmeio.value = 'Prùprio') and (tbvisitastipo.Value <> 'TERMO DE NOTIFICAùùO')            and
                                                 (tbvisitastipo.Value <> 'TERMO DE INTIMAùùO')              and
                                                 (tbvisitastipo.Value <> 'TERMO DE COLETA DE AMOSTRA')      and
                                                 (tbvisitastipo.Value <> 'CERTIDùO')                        and
                                                 (tbvisitastipo.Value <> 'AUTO DE INFRAùùO')                and
                                                 (tbvisitastipo.Value <> 'AUTO DE IMPOSIùùO DE PENALIDADE') then
        begin
            showmessage('Caro Usuùrio, meio de locomoùao incompatùvel com a atividade informada' );
            DBCMeio.SetFocus;
            exit;
        end;

{
ATO ADMINISTRATIVO DA GERùNCIA
CONFERùNCIA DE BALANùO
MANIFESTAùùO DO FISCAL ATUANTE
ORIENTAùùO SANITùRIA
ANùLISE DE PAS
PRORROGAùùO
RELATùRIO TùCNICO
RELATùRIO HARMONIZADO
 }
 ...
 //*******************************************
             ...
             begin
                ...
                begin
                     ...
                     begin
                        messagedlg('Informaùùes inconsistentes com outra entrada da tabela de inspeùùes. Autoridade: '+tbauxvisitafiscal1.value+', Inscriùùo: '+tbauxvisitacodigo.asstring+', Perùodo: '+tbauxvisitah_visita.asstring+' ùs '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' nù '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
//**********************************************
             ...
             begin
                ...
                begin
                     if (tbvisitash_visita.value > tbauxvisitah_visita.value) and (tbvisitash_visita.value < tbauxvisitah_fim.value) then
                     begin
                        messagedlg('Informaùùes inconsistentes com outra entrada da tabela de inspeùùes. Autoridade: '+tbauxvisitafiscal2.value+', Inscriùùo: '+tbauxvisitacodigo.asstring+', Perùodo: '+tbauxvisitah_visita.asstring+' ùs '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' nù '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
//**********************************************
             ...
             begin
                ...
                begin
                     if (tbvisitash_visita.value > tbauxvisitah_visita.value) and (tbvisitash_visita.value < tbauxvisitah_fim.value) then
                     begin
                        messagedlg('Informaùùes inconsistentes com outra entrada da tabela de inspeùùes. Autoridade: '+tbauxvisitafiscal3.value+', Inscriùùo: '+tbauxvisitacodigo.asstring+', Perùodo: '+tbauxvisitah_visita.asstring+' ùs '+ tbauxvisitah_fim.asstring+', Documento: '+tbauxvisitatipo.value+' nù '+tbauxvisitanumero.value,mtwarning,[mbok],0);
                        exit;
                     end;
...
{     if messagedlg('Deseja Salvar Conteùdo do '+#13+tbvisitastipo.value+' '+ tbvisitasnumero.AsString + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin}
...
{     end;}
       ...
       begin
           ...
           showmessage('Atenùùo! Terceiro fiscal nùo autorizado apra esta OS');
           dbcordem.SetFocus;
           exit
       end;
                      ...
                      begin
                          if (tbvisitastipo.Value <> 'CERTIDùO') or (tbreqcomplexidade.Value = 'BAIXA') then
                          begin
                              tbreq.Edit;
                              ...
                              tbreq.Post;
                              showmessage('Ordem de serviùo: ' + tbreqos.AsString+' Baixada com sucesso.');
                          end
                          else
                          begin
                              tbvisitaslibera.Value := 'Nùo';
                              showmessage('Atenùùo! A Ordem de Requerimento: ' + tbreqos.AsString+' nùo foi baixada pela certidùo emitida. Necessùrio reportar ù Gerùncia');
                          end;
                      end
                      else
                      begin
                          if tbvisitastipo.Value <> 'CERTIDùO' then
                          showmessage('Tudo bem! A OS '+ tbreqos.AsString+' jù foi baixada anteriormente.' )
                          else
                          begin
                              if tbreqcomplexidade.Value <> 'BAIXA' then
                              begin
                                  showmessage('Nùo pode ser emitida certidùo para Ordem de Requerimento : ' + tbreqos.AsString+' jù baixada.');
                                  DBCtipo.SetFocus;
                                  exit;
                              end;
                          end;
                    ...
                    begin
                        showmessage('Atenùùo. OS encaminhada para '+tbreqfiscal_encaminha.Value );
                        dbeos.SetFocus;
                        exit;
                    end;
                  end
                  ...
                  begin
                    showmessage('Requerimento ainda nùo ù uma OS encaminhada pelo titular de vigilùncia');
                    dbeos.SetFocus;
                    exit;
                  end;
                end
                ...
                begin
                    showmessage('OS nùo pertence a este regulado' );
                    dbeos.SetFocus;
                    exit;
                end;
            end
        ...
        begin
            ...
            begin
                ...
                begin
                    showmessage('Atenùùo! OS Cancelada '+ tboficiooficio.AsString);
                    dbeof.SetFocus;
                    exit;
                end;
                ...
                begin
                    showmessage('Atenùùo! Impossùvel registrar nesta data. A OS informada foi encaminhada em :'+ tboficiodtencaminha.AsString);
                    DBEDataV.SetFocus;
                    exit;
                end;
       ...
       begin
           ...
           exit
                ...
                begin
                    ...
                    begin
                        showmessage('Atenùùo! OS encaminhada ao fiscal '+tboficiofiscalencaminha.Value+'. Impossùvel continuar');
                        dbeof.SetFocus;
                        exit;
                    end;
                    if (tboficiomotivo.Value = 'SERVIùO TùCNICO') or (tboficiomotivo.Value = 'OPERAùùO FISCAL')  or (tboficiomotivo.Value = 'TREINAMENTO/REUNIùO')  then
                    begin
                        showmessage('Atenùùo! OS nùo direcionada a inspeùùo. Impossùvel continuar');
                        dbeof.SetFocus;
                        exit;
                    end;
                    ...
                    begin
                        ...
                        tboficio.Post;
                        showmessage('Ordem de serviùo: ' + tboficiooficio.AsString+' Baixada com sucesso.');
                    end
                    else
                    begin
                        showmessage('Atenùùo! OS '+ tboficiooficio.AsString+' jù baixada anteriormente.' );
                    end;
                end
                else
                begin
                    showmessage('Erro : tabela de OF # tabela de VF' );
                    dbeof.SetFocus;
                    exit;
                end;
            end
            ...
            begin
                ...
                begin
                    showmessage('Nù de protocolo nùo pertence ao regulado.');
                    dbeproto.SetFocus;
                    exit;
                end
//                else  showmessage('Achei protocolo '+inttostr(tbauxplantaprotocolo.value)+'  '+'regulado '+inttostr(tbauxplantacodigo.value))
            ...
            begin
                ...
                begin
                    ...
                    begin
                        showmessage('Atenùùo! Denùncia Arquivada :'+ tbdenunciadenuncia.AsString);
                        dbeden.SetFocus;
                        exit;
                    end;
                    if tbdenunciafiscalencaminha.Value <> frmlogin.c_user then
                    begin
                        showmessage('Atenùùo! Denùncia encaminhada para o fical :'+ tbdenunciafiscalencaminha.Value );
                        dbeden.SetFocus;
                        exit;
                    end;
                end
                ...
                begin
                    showmessage('Nù de denùncia nùo encontrado.');
                    dbeden.SetFocus;
                    exit;
                end;
            end;
...
//======================================================
              ...
              if (tbvisitasmodalidade.Value='DENùNCIA')     and (tbauxoficio.findkey(['DE'+inttostr(tbvisitasdenuncia.value)])) then retorno := true;
              if (tbvisitasmodalidade.Value='DE OFùCIO')    and (tbauxoficio.findkey(['OF'+inttostr(tbvisitasoficio.value)]))   then retorno := true;

              if retorno = false then
              begin
                  if tbvisitaslibera.Value = 'Sim' then
                  begin
                      if messagedlg('Confirma a geraraùùo automùtica do Retorno para a OS '+tbvisitasos.AsString+tbvisitasoficio.AsString+tbvisitasdenuncia.AsString+' ?',mtconfirmation,[mbyes,mbno],0)=mryes  then
                      begin
                          if  (tbvisitasmodalidade.value <> 'PROTOCOLO') then
                          begin
                                tboficio.Insert;
                                ...
                                tbOFICIOEMITENTE.value := 'GERENTE DE VIGILùNCIA SANITùRIA';
                                ...
                                if tbvisitasmodalidade.value = 'DENùNCIA'  then
                                    begin
                                        ...
                                        tboficioordem.value := 'Retorno da Denùncia '+tbvisitasdenuncia.AsString;
                                        tboficiofiscalencaminha.Value := tbdenunciafiscalencaminha.Value;
                                    end;
                                if tbvisitasmodalidade.value = 'DE OFùCIO' then
                                    begin
                                       tboficioorigem.value := 'OF'+inttostr(tbvisitasoficio.Value);
                                       tboficioordem.value := 'Retorno da Ordem de Ofùcio '+tbvisitasoficio.AsString;
              ...
              else showmessage('Atenùùo! Retorno para OS ' + tbvisitasos.AsString + tbvisitasoficio.AsString + tbvisitasdenuncia.AsString + ' jù foi gerado anteriormente .');
//=================================================
{
        ...
        end;
{
        ...
        begin
            showmessage('Caro Usuùrio, favor Informar o CNAE na ficha CAE');
            exit;
        end;
 }
  ...
  {       if tbcontribatividade.value = 0 then
    ...
    begin
      ...
      begin
         ...
         begin
            messagedlg('Atenùùo! Reportar-se ù gerencia devido a apontamento nesta data : '+datetostr(tbrdpfdata.value)+' - '+tbrdpfnome.value,mtwarning,[mbok],0);
            exit;
         end;
      end;
      tbrdpf.next;
...
{     if tbvisitasentrega.value = true then
     begin
        showmessage('Este documento jù estù anexado a um Relatùrio de Produùùo Fiscal jù entregue ù Adminstraùùo. Impossùvel Alterar !');
        exit;
     end; }
   ...
   begin
       if dayofweek(date) = 2 then
        begin
         ...
         begin
            showmessage('Perfil nùo autorizado alterar documento nesta data!');
...
{(tbvisitastipo.value='ATO ADMINISTRATIVO DA GERùNCIA')}
then
begin
...
showmessage('Perfil nùo autorizado!');
end;
...
//    =======================
     ...
     if messagedlg('Confirma a Exclusùo do Documento Nù:'+#13+ tbvisitasnumero.AsString + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
            ...
            if (daysbetween(date,tbvisitasdt_visita.value) > 1) then
               begin
                 showmessage('Impossùvel excluir a inspeùùo nesta data!');
                 DBEDataV.setfocus;
                 exit;
               end
               else
               begin
                    if not tbhistorico.Eof then tbhistorico.Delete else showmessage('Especificaùùes nùo digitada para este Documento!');
                    if not tbvisitas.Eof   then tbvisitas.Delete   else showmessage('Nùo Hù Documento a ser Excluùdo!');
                    Showmessage('Atenùùo. Lanùamento excluùdo. ù nessessùrio gerar novamente o Relatùrio de Produùùo para esta data');
               end;
             end
             else showmessage('Perfil nùo autorizado!');
     end;
...
begin
 showmessage('Alvarù nùo requerido para o exercùcio!');
 exit;
end;

if tbalvaralibera.value <> true then
begin
 showmessage('Alvarù nùo liberado para o exercùcio!');
 exit;
end;
painel:= 'alvara';
...
begin
 ...
 begin
   ...
   if (tbalvaraexercicio.value = yearof(date)) and (tbalvaracancela.Value = false) and (tbalvaratipo.Value = false) then
    begin
 //      if messagedlg('Confirma a Requerimento de mùltiplos alvarùs do contribuinte para o exercùcio: '+tbalvaraexercicio.AsString +'?',mtconfirmation,[mbyes,mbno],0)= mbno then exit;
        showmessage('Atenùùo! ùJù existe Alvarù cadastrado para o contribuinte no exercùcio');
        exit;
   end;
   tbalvara.insert;
...
{        if tbalvaranumero.value = 0 then
        begin
            showmessage('Caro Usuùrio, favor informar o Nùmero do Alvarù');
            exit;
        end;}

        if tbalvaraexercicio.isnull = true  then
        begin
            showmessage('Caro Usuùrio, favor informar o Exercùcio Fiscal');
            exit;
        end;

      if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvaradt_duam.Value) then
      begin
         showmessage('Data do Requerimento invùlida!');
         exit;
         DBEDtreq.setfocus;
      end;
 ...
 {        if tbalvaraexercicio.Value < 2022 then
   begin
      showmessage('Exercùcio do Requerimento Expirado!');
         exit;
         ...
         if tbalvaraexercicio.Value > 2022 then
   begin
      showmessage('Exercùcio do Requerimento Invùlido!');
         exit;
         DBCExercicio.setfocus;
   end;
        }
...
{   if tbalvaraexercicio.Value > strtoint(formatdatetime('yyyy',date)) then
    begin
      showmessage('Impossùvel requerer Alvarù pro exercùcio seguinte!');
         exit;
         DBCExercicio.setfocus;
    end;
 }
...
{        if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
        begin
                 showmessage('Usuùrio nùo autorizado a liberaùùo de  alvarù provisùrio!');
                 exit;
        end;    }
...
{        if tbalvaraunidade.value = ' ' then
        begin
            showmessage('Caro Usuùrio, favor informar o ùrea para a qual alvarù estù sendo liberado');
            exit;
        end;}
...
begin
     ...
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
             //    if not tbalvara.Eof then tbalvara.Delete else showmessage('Nùo Hù Alvarù Solicitado para este Contribuinte!');
                   if not tbalvara.Eof then
                   begin
                      // tbalvara.Delete
                      showmessage('Informar nas observaùùes o motivo do cancelamento');
                      tbalvara.edit;
                   ...
                   else showmessage('Nùo Hù Alvarù Solicitado para este Contribuinte!');
             end
             else showmessage('Cancelamento nùo autorizado');
     end;
     btemitenovo.setfocus;
end;
...
{    DBTtaxa.Visible:=true;}
    ...
    if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
    if tbcontribhorario.Value = 'NOTURNO' then Texto_horario2.Visible := true else Texto_horario2.Visible := false;
    if tbcontribpendoc.value = true then showmessage('AtenÁ„o: Contribuinte com pendÍncia no arquivo. Solicitar documentaùùo. Obrigado!          SETOR DE CADASTROS.       ');
    if tbcontribpessoa.value = 'CNPJ' then
        begin
...
{    DBTtaxa.Visible:=true;}
    ...
    if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
    if tbcontribhorario.Value = 'NOTURNO' then Texto_horario2.Visible := true else Texto_horario2.Visible := false;
    if tbcontribpendoc.value = true then showmessage('Atenùùo: Contribuinte com pendùncias no arquivo. Solicitar documentaùùo. Obrigado!          SETOR DE CADASTROS.       ');    
    if tbcontribpessoa.value = 'CNPJ' then
        begin
        DBECPF.Enabled  := false;
        DBECPF.Visible  := false;
...
begin
if tbcontribinatividade.value = true then TextoATV.Visible:= TRUE else TextoATV.Visible:= FALSE;
showmessage('Atenùùo! Alteraùùo do status de inatividade');
end;
...
begin
showmessage('Mùdulo em desenvolvimento!');
{if filtro = 0 then
begin
        showmessage('Atenùùo! Exibiùùo: somente INATIVOS');
        tbcontrib.Filter := 'inatividade = true';
        filtro := 1;
end
else
begin
        showmessage('Atenùùo! Exibiùùo: somente ATIVOS');
        tbcontrib.Filter := 'inatividade <> true';
        filtro := 0;
end;}
...
begin
        if messagedlg('Confirma a Liberaùùo Alvarù o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
begin
  if (frmlogin.c_grp = 'FIS') or (frmlogin.c_grp = 'ADM')  then
    begin
       ...
       showmessage('Alvarù liberado!');
    end
  else
     begin
       tbalvaralibera.Value := false;
       tbalvaraautoridade.value := ' ';
       showmessage('Funùao permitida somente para pessoal tùcnico');
     end
  end
...
if frmprincipal.tbsequenciadisponibilidade.value = false then showmessage('Caro Usuùrio, Sistema em manutenùùo, favor salvar os trabalhos e feche(SAIR) o WCVS');
end;
...
begin
{  if (tbvisitash_fim.value < tbvisitash_visita.value) and (tbvisitastipo.Value='CERTIDùO') and (tbvisitastipo.Value='CONTRA RAZùO') and (tbvisitastipo.Value='PRORROGAùùO') and (tbvisitastipo.Value='RùPLICA FISCAL') and (tbvisitastipo.Value='ATO ADMINISTRATIVO DA GERùNCIA') then
        begin
              showmessage('Atenùùo: Final da Inpeùùo Anterior ao Inùcio');
              exit;
        end;       }
//   if  SecondsBetween(tbvisitash_fim.value,tbvisitash_visita.value) > 10799 then showmessage('Atenùùo, inspeùùo de grande porte! Autorizaùùo da Gerùncia Necessùria.');
end;
...
begin
if (tbvisitastipo.Value = 'ATO ADMINISTRATIVO DA GERùNCIA') and (frmlogin.c_grp <> 'FIS') then
    tbvisitasfiscal1.value:= 'GERENTE DE VIGILùNCIA SANITùRIA';
end;
 ...
 exit;
  ...
  begin
    if frmlogin.c_grp = 'ADM' then
      begin
        if MessageDlg(
           'Atenùùo: os CNAEs do CVS divergem do arquivo do SIM.' + #13#10 +
           'Confirma a emissùo do alvarù assim mesmo?',
           mtConfirmation, [mbYes, mbNo], 0) = mrNo then
         Exit;
      end
      else
      begin
         ShowMessage('Emissùo bloqueada. Os CNAEs do CVS divergem do arquivo do SIM. Verifique a aba Complementar.');
         Exit;
      end;
    end;
{if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
begin
 showmessage('Usuùrio nùo autorizado a emitir alvarù provisùrio!');
 exit;
end;         }
  tbrt.First;
  while not tbrt.Eof do
...
begin
 showmessage('Exercùcio nùo requerido!');
 exit;
end;

if (tbalvlib.eof) and (tbalvlib.bof)  then
begin
 showmessage('Nùo existe atividade liberada para o exercùcio!');
 exit;
end;

if not tbalvaradt_emite.isnull then
begin
    showmessage('Alvarù jù Emitido!');
    exit;
end;
...
{if tbalvaranumero.value > 0 then
begin
    showmessage('Alvarù Jù expedido. Escolha Reemissùo!');
    exit;
end;}
  ...
  begin
    if frmlogin.c_grp = 'ADM' then
      begin
        if MessageDlg(
           'Atenùùo: os CNAEs do CVS divergem do arquivo do SIM.' + #13#10 +
           'Confirma a reemissùo do alvarù assim mesmo?',
           mtConfirmation, [mbYes, mbNo], 0) = mrNo then
         Exit;
      end
      else
      begin
         ShowMessage('Reemissùo bloqueada. Os CNAEs do CVS divergem do arquivo do SIM. Verifique a aba Complementar.');
         Exit;
      end;
    end;
...
begin
    showmessage('Alvarù nùo Emitido!');
    exit;
end;
...
begin
  ...
  begin
      ...
      begin
          ...
          begin
             showmessage('Hù pelo menos uma atividade econùmica cadastrada com a subclasse nùo informada, selecione alterar e complete o cadastro antes de incluir outra atividade econùmica!');
             exit;
          end;
          tbauxrt.Next;
      end;
...
begin
  ...
  begin
      if messagedlg('Confirma a Exclusùo da Atividade: '+#13+tbrtsubclasse.value+' - '+tbcnaeatividade.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbrt.delete;
  end
  else showmessage('Perfil Nùo Autorizado!');
...
begin
  ...
  if tbrtsubclasse.isnull  then
  begin
     showmessage('Favor informar o cùdigo CNAE!');
     exit;
  end;

  if (tbrttipo.Value <> 'Principal') and (tbrttipo.Value <> 'Secundùria')then
  begin
     showmessage('Favor selecionar se a Atividade ù Principal ou Secundùria!');
     exit;
  end;
  ...
  begin
      ...
      begin
          showmessage('Atividade jù cadastrada para contribuinte!');
          exit;
      end;
      if (tbrttipo.value = 'Principal') and (tbauxrttipo.value = 'Principal') and (tbrtcontrole.value <> tbauxrtcontrole.value) then
      begin
          showmessage('Atividade Principal jù cadastrada para contribuinte!');
          exit;
      end;
      tbauxrt.Next;
  end;
...
{        if tbcontribatividade.value = 0 then
    ...
    begin
            if messagedlg('Atenùùo!!!!! Exercùcio anterior. Confirma liberaùùo para o ano PASSADO ?  ',mtconfirmation,[mbyes,mbno],0)=mrno then  exit;
    //        showmessage('Exercùcio Invùlido');
//        exit;
    end;
...
begin
  ...
  begin
         showmessage('Caro Usuùrio, favor Informar a atividade a ser liberada.');
         exit;
  end;
  cae2:= false;
  tbauxrt.First;
 ...
 {     if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvlibData.Value) then
      begin
         showmessage('Exercùcio da liberaùùo diferente do aexercùocio requerido');
         exit;
  ...
  begin
          ...
          begin
              showmessage('Atenùùo! Informar a autoridade que liberou a atividade econùmica');
              DBCAutoridade.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Atenùùo! Informar O documento de liberaùùo da atividade econùmica');
              DBCAutoridade.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Atenùùo! Informar a data da liberaùùo da atividade econùmica');
              DBCAutoridade.SetFocus;
              exit;
          end;
      ...
      //                 if (tbvisitaslibera.Value = 'Liberado') or (tbvisitaslibera.Value = 'Alguns CNAEs') then showmessage('ok');
                         if (tbvisitaslibera.Value = 'Nùo Liberado') then
                         begin
                             showmessage('Atenùùo! A Autoridade Sanitùria relatou nùo conformidade na inspeùùo')
                         end;
                         digitado := true;
                     end;
                 end;
          ...
          if digitado = false then showmessage ('Atenùùo!!! Documento de liberaùùo da atividade econùmica nùo localizado. Click em Ok para continuar ...');

  end
  else showmessage('Baixa Complexidade. Liberaùùo Automùtica');
...
begin
    ...
    begin
      showmessage('Atividade Econùmica jù liberada para o exercùcio!');
      exit;
    end;
    tbauxlib.next;
end;
   ...
   tbalvlib.post;
   showmessage('Autorizaùùo de Atividade Concluùda!');
end;
...
begin
     if messagedlg('Confirma a Exclusùo da Atividade: '+tbalvlibatividade.value+#13+' do Alvara: '+tbalvaranumero.AsString + ' de '+tbalvaraexercicio.AsString +'?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
                 if not tbalvaradt_emite.isnull then
                 begin
                     showmessage('Nùo ù possùvel excluir. Alvarù jù emitido!');
                     exit;
                 end;
                 if not tbalvlib.Eof then tbalvlib.Delete else showmessage('Nùo Hù Atividade Liberada para este Contribuinte!');
             end
             else showmessage('Exclusùo nùo autorizada');
     end;
end;
...
{      if (frmlogin.c_grp = 'CAD') or (frmlogin.c_grp = 'CON') then
      begin
        if tbveiculoautorizacao.Value = true then
          begin
            showmessage('Usuùrio nùo autorizado a libeùùo veicular!');
            exit;
          end;
      end;}
        tbveiculo.post;
...
begin
     if messagedlg('Confirma a Exclusùo do Alvarù '+tbveiculoexercicio.AsString +' do Veùculo '+ tbveiculomarca.value + ' Placa '+ tbveiculoplaca.value + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
                 if not tbveiculo.Eof then tbveiculo.Delete else showmessage('Nùo Hù Alvarù Veicular Solicitado para este Contribuinte!');
             end
             else showmessage('Exclusùo nùo autorizada');
     end;
     btimprime.setfocus;
...
begin
    ...
    begin
       showmessage('Alvarù Veicular nùo requerido!');
       exit;
    end;

    if tbveiculoautorizacao.Value <> true then
    begin
      showmessage('Alvarù Veicular nùo autorizado!');
      exit;
    end;
...
{        if (frmlogin.c_grp <> 'ADM') and (tbalvaratipo.Value = true) then
        begin
          showmessage('Usuùrio nùo autorizado ù liberaùùo de Alvarù Provisùrio!');
...
begin
  ...
  if (frmlogin.c_grp = 'FIS') or (frmlogin.c_grp = 'ADM')  then
  begin
      if tbveiculoautorizacao.Value = true then showmessage('Alvarù Veicular Jù Liberado')
      else
      begin
          if messagedlg('Confirma a Liberaùùo do Alvarù '+tbveiculoexercicio.AsString +' para o Veùculo '+ tbveiculomarca.value + ' Placa '+ tbveiculoplaca.value + '?',mtconfirmation,[mbyes,mbno],0)=mryes then
             begin
                 tbveiculo.edit;
...
begin
  ...
  begin
      ...
      begin
          if not tbalvaradt_emite.isnull then
          begin
                 showmessage('Jù existe Alvarù emitido para o contribuinte no exercùcio');
                 exit;
          end;
      end;
   ...
   begin
        showmessage('impossùvel emitir alvarù provisùrio para este exercùcio');
        exit;
   end;

      if messagedlg('Confirma a Liberaùùo Provisùria para o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
      begin
              tbalvara.edit;
              tbalvaratipo.Value:=true;
              tbalvara.Post;
              showmessage('Alvarù Provisùrio Liberado');
      end;
  end
  else
  begin
...
begin
  ...
  begin
   ...
   if not tbalvaraexercicio.value = yearof(date) then
   begin
        showmessage('impossùvel emitir alvarù eventual para este exercùcio');
        exit;
   end;

      if messagedlg('Confirma a Liberaùùo de Alvarù de Evento para o Contribuinte'+#13+tbcontribRAZAO.value+'?',mtconfirmation,[mbyes,mbno],0)=mryes then
      begin
              tbalvara.edit;
              tbalvaraevento.Value :=true;
              tbalvara.Post;
              showmessage('Alvarù Eventual Liberado');
      end;
  end                                                                       
  else
  begin
...
//  begin
//       showmessage('Data da Validade invùlida!');
//        DBEDtreq.setfocus;
//   end;
...
{//   if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvaradt_duam.Value) then
   ...
   begin
      showmessage('Data do Requerimento invùlida - PASSADO!');
            DBCExercicio.setfocus;
   end;
  }
   ...
   if tbalvaraexercicio.Value > strtoint(formatdatetime('yyyy',date)) then
    begin
      showmessage('Data do Requerimento invùlida - FUTURO!');
         DBEDuam.setfocus;
   end;
end;
...
begin
   ...
   begin
      showmessage('Data da liberaùùo invùlida!');
        DBEDtlibera.setfocus;
   end;
end;
...
// TbClientes ù o nome de alguma Tabela
// TbClientesCli_Foto ù um variavel da tabela do tipo Blob
// Image1 ù um componente
// OpenPictureDialog1 ù o componente para abrir a figura
end;
///====== teste imagem inicio
   }
...
begin
 //   if pagecontrol1.activePage = TabAnexo   then showmessage('Mùdulo em fase de testes');
    if pagecontrol1.activePage = TabPlanta  then
    begin
//        showmessage('Atenùùo!!! Conferir o nùmero do processo (EM VERMELHO) que estù tramitando/consultanto');
        tbplanta.Filtered := false;
    end;
    if pagecontrol1.activePage = TabProjeto then
    begin
        tbplanta.Filtered := true;
        tbplanta.Filter := 'assunto='+quotedstr('PROJETO ARQUITETùNICO SANITùRIO');
    end;
    if pagecontrol1.activePage = TabPas then
    begin
        tbplanta.Filtered := true;
        tbplanta.Filter := 'assunto='+quotedstr('PROCESSO ADMINISTRATIVO SANITùRIO');
    end;
    if pagecontrol1.activePage = TabCan then
    begin
        tbplanta.Filtered := true;
        tbplanta.Filter := 'assunto='+quotedstr('REGULARIDADE DE PRODUTO ALIMENTùCIO');
    end;
    ...
    begin;
        showmessage('Atenùùo, salvar a ediùùo antes de mudar de aba. Os dados nùo salvos serùo perdidos. Cancelando a operaùùo...');
        tbtramitacao.Cancel;
        tbplanta.cancel;
        tbveiculo.Cancel;
        tbalvlib.Cancel;
...
begin
  if (tbplanta.Bof) and (tbplanta.Eof) then showmessage('Nùo existe protocolo pra este regulado') else
  begin
    if (frmlogin.c_grp = 'ADM') or (frmlogin.c_grp = 'CAD') or (frmlogin.c_per = 'ORD') then
    begin
        if  (tbplantacarga.Value = 'PROTOCOLO') or (tbplantacarga.Value = 'ARQUIVO') or (tbplantacarga.Value = 'OUTRA REPARTIùùO') or (tbplantacarga.Value = 'SERVIùO DE NOTIFICAùùO') or (tbplantacarga.Value = 'TURMA JULGADORA') or (tbplantacarga.Value = 'GERùNCIA') or (tbplantacarga.Value = 'DIRETORIA') or (tbplantacarga.Value = 'NùCLEO DE ENGENHARIA') or (tbplantacarga.Value = 'NùCLEO DO PAS') or (tbplantacarga.Value = 'NùCLEO DO RPA') or (tbplantacarga.Value = frmlogin.c_user) then
        begin
            tbtramitacao.insert;
            ...
            if (frmlogin.c_grp = 'ADM')  then
            begin
                showmessage('Atenùùo usuùrio administrador do sistema, o processo selecionado estù com carga para :   '+tbplantacarga.Value+'. Pra tramitaùùo forùada, utilize a funùùo JUNTADA, informe o despacho justificando o motivo da quebra de fluxo processual no sistema e anexe-o ao processo fùsico, .');
               { tbtramitacao.insert;
                tbtramitacaousuario.Value  := frmlogin.c_user;
                tbtramitacaodata.value     := date;
                tbtramitacaohora.Value     := time;
            ...
            else showmessage('Somente o usuùrio com a carga do processo pode tramitar:   '+tbplantacarga.Value);
    end
    else
    begin
        ...
        begin
           ...
           showmessage('A carga do processo nùo estù com o usuùrio');
        end;
    end;
  end;
end;
...
begin
    ...
    begin
          ...
          begin
             showmessage('Obrigatùrio Escolher o Assunto');
             dbcassunto.SetFocus;
             exit;
          end;
          if tbplantacomplemento.IsNull then
          begin
              showmessage('Obrigatùrio Informaùùes Complementrares do Processo');
          ...
          begin
              showmessage('Obrigatùrio Informar o nome completo da pessoa que estù dando entrada no processo');
              DBEptnome.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Obrigatùrio Informar um documento de identificaùùo da pessoa que estù dando entrada no processo');
              DBEdoc.SetFocus;
              exit;
          end;
          ...
          begin
              showmessage('Obrigatùrio Informar o celular da pessoa que estù dando entrada no processo');
              DBEcelular.SetFocus;
              exit;
          end;
    ...
    begin
        if tbtramitacaodescricao.IsNull then
          begin
             showmessage('Obrigatùrio Selecioanr a Descriùùo da fase');
             DBCdescric.SetFocus;
             exit;
          end;
          if tbtramitacaodestino.IsNull then
          begin
             showmessage('Obrigatùrio Selecionar a destinaùùo do processo');
             DBCdestino.SetFocus;
             exit;
          end;
          ...
          begin
                ...
                begin
                    showmessage('Obrigatùrio Informar o documento que estù sendo juntado ao processo');
                    DBCJuntada.SetFocus;
                    exit;
                end;
                if tbtramitacaointeressado.IsNull then
                begin
                    showmessage('Obrigatùrio Informar o nome da pessoa que estù juntando documento ao processo');
                    DBEInteressado.SetFocus;
                    exit;
                end;
                if (tbtramitacaojuntada.Value = 'Despacho da Gerùncia') and (frmlogin.c_grp <> 'ADM') then
                begin
                  showmessage('Opùùo invùlida para o perfil');
                  DBCJuntada.SetFocus;
                  exit;
                end;
          end;
...
begin
    showmessage('funùùo nùo permitida para o perfil');
end;
...
begin
  if (tbtramitacao.Bof) and (tbtramitacao.Eof) then showmessage('Nùo existe tramitaùùo pra este regulado') else
  begin
    if tbtramitacaousuario.Value <> frmlogin.c_user then showmessage('Somento o usuùrio que tramita pode emitir o termo de resssa') else
    begin
      if tbtramitacaojuntada.IsNull then
      begin
          painel := 'protocolo';
      ...
      else showmessage('O Registro selecionado nùo ù uma Tramitaùùo. ù uma juntada de documentos');
    end;
  end;
end;
...
if (tbplanta.Bof) and (tbplanta.Eof) then showmessage('Nùo existe protocolo pra este regulado') else
    begin
        ...
        begin
          ...
          else showmessage('Processo com carga para fiscalizaùùo. impossùvel efetuar juntada de documento');
        end
        else
        if (frmlogin.c_grp = 'ADM') then
        begin
...
begin
  ...
  begin
   ...
   begin
     if tbtramitacaojuntada.IsNull then showmessage('O Registro selecionado nùo ù uma Juntada de documentos. ù uma tramitaùùo') else
     begin
        painel := 'protocolo';
...
begin
    ...
    if tbvisitasmodalidade.value = 'DENùNCIA'     then DBEDen.setfocus;
    if tbvisitasmodalidade.value = 'DE OFùCIO'    then DBEOf.setfocus;
end;
...
begin
  ...
  begin
    // Aqui vocù jù tem o arquivo escolhido
    ShowMessage('Vocù escolheu: ' + OpenDialog1.FileName);

    // Se vocù estiver usando o AcroPDF:
...
begin
  ...
  try
    ...
    ShowMessage('Exportaùùo concluùda!');
  finally
...
begin
  ShowMessage('Erro CSV (Requerimento/Tramitaùùo/Ofùcio/Denùncia/Planta):' + sLineBreak + Erro);
  Exit;
end;
...
begin
  ShowMessage('Erro CSV (Alvarù/Liberaùùo):' + sLineBreak + Erro);
  Exit;
end;


{    // Gera TRAMITACAO ENXUTA (sobrescreve tramitacao.csv com filtro + ùltimo por protocolo)
    if not ExportarTramitacaoEnxuta('\\192.168.1.2\wcvs\repo\docs\data\',
      tbtramitacao, frmlogin.tbLogin, Erro) then
    begin
      ShowMessage('Erro CSV (Tramitaùùo Enxuta):' + sLineBreak + Erro);
      Exit;
    end;     }
...
begin
  ...
  begin
    ...
    FSimStatus.Left := 8;
    FSimStatus.Top := 16;
    FSimStatus.Caption := 'API SIM: n„o disponÌvel';
    FSimStatus.Font.Style := [fsBold];
  end;

  if FSimProgress = nil then
  begin
    FSimProgress := TProgressBar.Create(Self);
    FSimProgress.Parent := GroupBox13;
    FSimProgress.Left := 8;
    FSimProgress.Top := 34;
    FSimProgress.Width := GroupBox13.Width - 16;
    FSimProgress.Height := 16;
    FSimProgress.Anchors := [akLeft, akTop, akRight];
    FSimProgress.Visible := False;
  end;
  ...
  begin
    ...
    FSimGrid.Height := GroupBox13.Height - 62;
...
begin
  ...
  FSimGrid.Cells[1,0] := 'DescriÁ„ùùo';
...
function Tfrmestabe.CaminhoSIMAux: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIM\cnae_aux.csv';
end;

function Tfrmestabe.CaminhoSIMLock: string;
begin
  Result := IncludeTrailingPathDelimiter(AliasPath('WCVS')) + 'SIM\cnae_aux.lock';
end;
...
var
  ...
  T: TTable;
begin
  { Regenerar SIMAUX quando: tabela nùo existe, tabela vazia, ou cnae.csv mais novo que a tabela }
  Result := True;
  if not FileExists(CaminhoSIMBruto) then Exit;
  if not FileExists(CaminhoSIMTabela) then Exit;

  { Tabela vazia (ex.: criada ù mùo ou por CriarSIMAUX) deve ser preenchida pelo cnae.csv }
  T := TTable.Create(nil);
  try
    T.DatabaseName := 'wcvs';
  ...
  { Detectar alteraùùo de data/hora do cnae.csv: se o CSV for mais novo que a tabela, regenerar }
...
begin
  ...
  try
    ...
    try
      ...
      if Assigned(FSimProgress) then
      begin
        FSimProgress.Min := 0;
        FSimProgress.Max := LinhaCount;
        FSimProgress.Position := 0;
        FSimProgress.Visible := True;
      end;
      ...
      try
        ...
        begin
          ...
          if (LinhaAtual mod 200 = 0) then
          begin
            if Assigned(FSimProgress) then
              FSimProgress.Position := LinhaAtual;
            if Assigned(FSimStatus) then
              FSimStatus.Caption := 'SIM: processando ' + IntToStr(LinhaAtual) + ' de ' + IntToStr(LinhaCount) + ' linhas...';
            Application.ProcessMessages;
          end;
        ...
        if Assigned(FSimProgress) then
          FSimProgress.Position := LinhaCount;
        if Assigned(FSimStatus) then
        begin
          FSimStatus.Caption := 'SIM: reconstruindo indices (' + IntToStr(RegCount) + ' registros)...';
          Application.ProcessMessages;
        end;
...
begin
  ...
  try
    ...
    begin
      AtualizarStatusSincronizacaoSIM;
      FSimStatus.Caption := FSimStatus.Caption + ' | Cadastro no localizado';
      FSimGrid.RowCount := 1;
      FSimDivergente := True;
      Application.ProcessMessages;
      Exit;
    ...
    begin
      ...
      begin
        ...
        FSimGrid.Cells[2, FSimGrid.RowCount - 1] := 'NùO COMPETE';
    end;
...
function Tfrmestabe.PrecisaGerarAuxSIM: Boolean;
var
  DtBruto, DtAux: Integer;
begin
  Result := True;

  if not FileExists(CaminhoSIMBruto) then Exit;
  if not FileExists(CaminhoSIMAux) then Exit;

  DtBruto := FileAge(CaminhoSIMBruto);
  DtAux   := FileAge(CaminhoSIMAux);

  if (DtBruto <> -1) and (DtAux <> -1) and (DtAux >= DtBruto) then
    Result := False;
end;

function Tfrmestabe.GerarAuxSIM: Boolean;
var
  ArqIn, ArqOut, ArqLock: TextFile;
  L, SitCSV, MunCSV, CNAECSV: string;
  Bloco: TStringList;
  ListaVisa: TStringList;
  TemCnaeVisa, BlocoAtivo: Boolean;

  procedure GravarBloco;
  var
    j: Integer;
  begin
    if BlocoAtivo and TemCnaeVisa and (Bloco.Count > 0) then
    begin
      for j := 0 to Bloco.Count - 1 do
        WriteLn(ArqOut, Bloco[j]);
    end;
  end;
...
begin
  ...
  if FileExists(CaminhoSIMLock) then
    Exit;
  ...
  begin
    ...
    AssignFile(ArqLock, CaminhoSIMLock);
    Rewrite(ArqLock);
    CloseFile(ArqLock);
    ...
    AssignFile(ArqOut, CaminhoSIMAux);
    Rewrite(ArqOut);
    try
      if not EOF(ArqIn) then
      begin
...
begin
  ...
  if FileExists(CaminhoSIMAux) then
  begin
    DtAux := FileAge(CaminhoSIMAux);
    if DtAux <> -1 then
    begin
...
var
  ListaSIMCNAE : TStringList;
  ListaSIMDESC : TStringList;
  ListaRT      : TStringList;
  Arq          : TextFile;
  L            : string;
  DocCSV       : string;
  ...
  i            : Integer;
  AchouInscricao: Boolean;
  AchouDocumento : Boolean;


  procedure AdicionarOuAtualizarCNAE(const ACNAE, ADescricao: string);
  var
    p: Integer;
  begin
    if ACNAE = '' then Exit;

    p := ListaSIMCNAE.IndexOf(ACNAE);
    if p < 0 then
    begin
      ListaSIMCNAE.Add(ACNAE);
      ListaSIMDESC.Add(ADescricao);
    end
    else if (Trim(ListaSIMDESC[p]) = '') and (Trim(ADescricao) <> '') then
      ListaSIMDESC[p] := ADescricao;
  end;
...
begin
  ...
  begin
    ...
    else if FileExists(CaminhoSIMTabela) then
    begin
      ValidarCNAESIM_Tabela;
      Exit;
    end;
      ...
      on E: EDBEngineError do
        { Tabela SIMAUX inexistente ou inacessùvel; segue para o CSV }
        ;
    end;
  end;
  ...
  if PrecisaGerarAuxSIM then
  begin
    FSimStatus.Caption := 'API SIM: atualizando...';
    Application.ProcessMessages;
    GerarAuxSIM;
  end;
  ...
  try
    if not FileExists(CaminhoSIMAux) then
    begin
      FSimStatus.Caption := 'API SIM: auxiliar nùo encontrado';
      Exit;
    end;
    ...
    AssignFile(Arq, CaminhoSIMAux);
    Reset(Arq);
    try
      if not EOF(Arq) then
        ReadLn(Arq, L); // cabeùalho
      ...
      { 1a busca: por inscriùùo ù percorre todo o arquivo (auxiliar nùo estù ordenado) }
      if MunAtual <> '' then
      begin
        ...
        begin
          ...
          MunCSV   := ApenasNumeros(Trim(CampoCSV(L, 0)));
          RazaoCSV := Trim(CampoCSV(L, 2));
          CNAECSV  := Trim(CampoCSV(L, 3));
          AtivCSV  := Trim(CampoCSV(L, 6));
          { Nùo exige SitCSV = ATIVO no auxiliar; o arquivo jù foi filtrado na geraùùo }

          if MunCSV <> '' then
            if MunCSV = MunAtual then
            begin
              AchouInscricao := True;
              TipoMatch := 'Inscriùùo';
              ValorMatch := MunCSV;
              RazaoMatch := RazaoCSV;
              AdicionarOuAtualizarCNAE(CNAECSV, AtivCSV);
            end;
        end;
      end;
      { 2a busca: por documento, somente se nùo achou inscriùùo }
      if (not AchouInscricao) and (DocAtual <> '') then
      begin
        CloseFile(Arq);
        AssignFile(Arq, CaminhoSIMAux);
        ...
        begin
          ...
          DocCSV  := ApenasNumeros(Trim(CampoCSV(L, 5)));
          RazaoCSV := Trim(CampoCSV(L, 2));
          CNAECSV := Trim(CampoCSV(L, 3));
          AtivCSV := Trim(CampoCSV(L, 6));
          { Nùo exige SitCSV = ATIVO no auxiliar; o arquivo jù foi filtrado na geraùùo }

          if DocCSV = DocAtual then
          begin
            AchouDocumento := True;
            TipoMatch := 'CNPJ/CPF';
            ValorMatch := DocCSV;
    ...
    begin
      ...
      FSimGrid.RowCount := 1;
      FSimDivergente := True;
      FSimStatus.Caption := FSimStatus.Caption + ' | Cadastro nùo localizado';
      Application.ProcessMessages;
      Exit;
    end;
    ...
    begin
      if RazaoMatch <> '' then
        FSimStatus.Caption := FSimStatus.Caption + ' | ' + TipoMatch + ': ' + ValorMatch + ' | ' + RazaoMatch
      else
        FSimStatus.Caption := FSimStatus.Caption + ' | ' + TipoMatch + ': ' + ValorMatch;
    end
    else
      FSimStatus.Caption := FSimStatus.Caption + ' | Cadastro n„ùo localizado';
