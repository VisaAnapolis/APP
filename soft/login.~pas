...
//      tblogin.FindKey([cusuarios.text]);
        // Migracao silenciosa: se senha em plain text e correta, regrava em hash
        if (Length(tbloginsenha.value) < 64) and (tbloginsenha.value = Esenha.Text) then
        begin
          tblogin.Edit;
          tbloginsenha.value := HashSenha(Esenha.Text);
          // Se nunca trocou (senha2 vazia), considera senha expirada
          if tbloginsenha2.value = '' then
            tblogindata.Value := Date - 31;
          tblogin.Post;
        end;
        if tbloginsenha.value = HashSenha(Esenha.Text) then
          begin
              // Usuário desabilitado
              if tblogingrupo.value = 'DES' then
              begin
                showmessage('Usuário Desabilitado, procure o administrador.');
                Esenha.SetFocus;
                exit;
              end;

              // Senha expirada (>= 31 dias): obriga troca
              if daysbetween(date, tblogindata.value) >= 31 then
              begin
                showmessage('Sua senha expirou e deve ser alterada para continuar.' + #13 +
                            'Informe a nova senha no campo "Nova Senha" e clique em Alterar.');
                Enova.SetFocus;
                exit;
              end;

              // Aviso de proximidade (26 a 30 dias): sugere troca
              if daysbetween(date, tblogindata.value) >= 26 then
              begin
                if messagedlg('Sua senha expira em ' +
                   IntToStr(31 - daysbetween(date, tblogindata.value)) +
                   ' dia(s). Deseja alterar agora?' + #13,
                   mtconfirmation, [mbyes, mbno], 0) = mryes then
                begin
                  showmessage('Informe a nova senha no campo "Nova Senha" e clique em Alterar.');
                  Enova.SetFocus;
                  exit;
                end;
              end;
...
if Enova.Text = '' then
    ...
    begin
//        tblogin.FindKey([cusuarios.text]);
        if SenhasIguais(Esenha.Text, tbloginsenha.value) then
        begin
          // Usuário desabilitado não pode alterar senha
          if tblogingrupo.value = 'DES' then
          begin
            showmessage('Usuário Desabilitado, procure o administrador.');
            exit;
          ...
          if SenhasIguais(Enova.Text, tbloginsenha.value) or
             SenhasIguais(Enova.Text, tbloginsenha2.value) or
             SenhasIguais(Enova.Text, tbloginsenha3.value) then
          begin
...
begin
    ...
    begin
        ...
        tbloginsenha.value:=HashSenha(Enova.text);
