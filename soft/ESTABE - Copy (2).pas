...
begin
  ...
  if (FTSimAux = nil) and FileExists(CaminhoSIMTabela) then
  begin
    FTSimAux := TTable.Create(Self);
    FTSimAux.DatabaseName := 'wcvs';
    FTSimAux.TableName    := 'SIMAUX.DB';
...
begin
  ...
  try
    ...
    while not T.Eof do
    begin
      Lin := CsvCampoSim(Trim(T.FieldByName('INSCRICAO').AsString)) + ';' +
             ';' +
             CsvCampoSim(Copy(Trim(T.FieldByName('RAZAO').AsString), 1, 254)) + ';' +
             CsvCampoSim(Trim(T.FieldByName('CNAE').AsString)) + ';' +
             ';' +
             CsvCampoSim(Trim(T.FieldByName('DOCUMENTO').AsString)) + ';' +
             CsvCampoSim(Copy(Trim(T.FieldByName('ATIVIDADE').AsString), 1, 254)) + ';' +
             'Ativo;' +
             ';' +
             ';' +
             ';' +
             CsvCampoSim(Trim(T.FieldByName('IND_PRINCIPAL').AsString));
      SL.Add(Lin);
      T.Next;
    end;
    T.Close;
...
begin
  ...
  { Checar datas primeiro: se cnae.csv mais novo, regenerar sem abrir a tabela }
  DtBruto := FileAge(CaminhoSIMBruto);
  DtTabela := FileAge(CaminhoSIMTabela);
  if (DtBruto <> -1) and (DtTabela <> -1) and (DtBruto > DtTabela) then
    Exit;

  { Datas indicam tabela atualizada; verificar se Não esta vazia }
  T := TTable.Create(nil);
  try
    T.DatabaseName := 'wcvs';
    T.TableName := 'SIMAUX.DB';
...
var
  ...
  begin
    ...
    begin
      ...
      T.FieldByName('DOCUMENTO').AsString     := ApenasNumeros(Trim(CampoCSV(Lin, 3)));
      T.FieldByName('RAZAO').AsString         := Copy(Trim(CampoCSV(Lin, 2)), 1, 100);
      T.FieldByName('CNAE').AsString          := Trim(CampoCSV(Lin, 4));
      T.FieldByName('ATIVIDADE').AsString     := Copy(Trim(CampoCSV(Lin, 6)), 1, 254);
      T.FieldByName('IND_PRINCIPAL').AsString := Trim(CampoCSV(Lin, 5));
      T.FieldByName('ENDERECO').AsString := Copy(Trim(CampoCSV(Lin, 9)), 1, 254);
      T.FieldByName('CEP').AsString      := Trim(CampoCSV(Lin, 10));
...
try
  ...
  { Sempre recriar do zero: garante arquivos de indice limpos }
  FreeAndNil(FTSimAux);
  ...
  T.FieldDefs.Add('IND_PRINCIPAL', ftString,  1,   False);
  T.FieldDefs.Add('ENDERECO', ftString, 254, False);
  T.FieldDefs.Add('CEP',      ftString, 10,  False);
  T.IndexDefs.Clear;
  T.IndexDefs.Add('', 'CONTROLE', [ixPrimary, ixUnique]);
  T.IndexDefs.Add('INSCRICAO', 'INSCRICAO', [ixCaseInsensitive]);
  T.IndexDefs.Add('DOCUMENTO', 'DOCUMENTO', [ixCaseInsensitive]);
  T.CreateTable;
  
  T.Open;
    { Sem loop de Delete - tabela ja esta vazia e com indices limpos  }
...
begin
  ...
  begin
    ShowMessage('Tabela SIMAUX Não encontrada. Execute a validação na aba Complementar primeiro.');
    Exit;
  end;
  ...
  try
    ...
    try
      ...
      begin
        TSim.Filter := 'DOCUMENTO=' + QuotedStr(DocAtual);
        TSim.Filtered := True;
        TSim.First;
        while not TSim.Eof do
        begin
...
begin
  ...
  try
    ...
    try
      ...
      TemIdx := (TSim.IndexDefs.IndexOf('INSCRICAO') >= 0) and
                (TSim.IndexDefs.IndexOf('DOCUMENTO') >= 0);
...
//      T0 := GetTickCount;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            EnderecoSIM    := Trim(TSim.FieldByName('ENDERECO').AsString);
            CEPSIM         := Trim(TSim.FieldByName('CEP').AsString);
          end;
      ...
      begin
        if TemIdx then
        begin
          TSim.IndexName := 'DOCUMENTO';
          TSim.SetRange([DocAtual], [DocAtual]);
        end
        else
        begin
          TSim.Filter := 'DOCUMENTO=' + QuotedStr(DocAtual);
...
//      ShowMessage('SIMAUX Filter levou: ' + IntToStr(GetTickCount - T0) + ' ms');
...
begin
  ...
  begin
    try
      if FileExists(CaminhoSIMTabela) and not PrecisaGerarSIMTabela then
      begin
        ValidarCNAESIM_Tabela;
        Exit;
      end;
...
begin
  ...
  SIMAUXEmUso := TabelaEmUso('SIMAUX.DB');

  if TaxaEmUso or SIMAUXEmUso then
  begin
    TabelasEmUso := '';
    if TaxaEmUso   then TabelasEmUso := TabelasEmUso + '  - SIMTAXA.DB' + #13#10;
    if SIMAUXEmUso then TabelasEmUso := TabelasEmUso + '  - SIMAUX.DB'  + #13#10;
    ShowMessage(
  ...
  if MessageDlg(
    'Isso vai recriar as tabelas SIMTAXA.DB e SIMAUX.DB.' + #13#10 +
