...
{    if (tbsinavisacomplexidade.value <> complex) and (complex <> '' ) and (frmlogin.c_grp <> 'ADM') then
         begin
            showmessage('Alteracao da Complexidade do Contribuinte Não autorizada para o usuario.');
            DBLGrupo.setfocus;
            exit;
      end;}
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' ja cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Usuario Não autorizado',mtwarning,[mbok],0);
//            if messagedlg('CNPJ: '+tbauxcontribcgc.asstring+' ja cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+#13+'Confirma a multiplicacao do cadastro'+'?',mtconfirmation,[mbyes,mbno],0)=mryes then tbcontribrazao.value:=tbauxcontribrazao.value else exit;
    ...
    begin
       ...
       begin
         ...
         begin
          messagedlg('CPF: '+tbauxcontribcpf.asstring+' ja cadastrado! Contribuinte: '+tbauxcontribcodigo.asstring+'-'+tbauxcontribrazao.value+' - Perfil Não autorizado',mtwarning,[mbok],0);
          exit;
         end
...
case key of
        ...
        begin
            if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F3 Não suportada em modo edicao');
        end;
        vk_F2:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F2 Não suportada em modo edicao');
        end;
        vk_F1:
        begin
         if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F1 Não suportada em modo edicao');
        end;
        vk_F4:
        begin
             if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                      ...
                      showmessage('F4 Não suportada em modo edicao');
        end;
        vk_F5:
        begin
        if (tbvisitas.state=dsbrowse) and (tbalvara.state=dsbrowse) and (tbcontrib.state=dsbrowse) and (tbveiculo.state=dsbrowse) then
                    PageControl1.ActivePage := TabAlvara
                      ...
                      showmessage('F5 Não suportada em modo edicao');
        end;
...
retorno := false;
      perform(Wm_nextdlgctl,0,0);
       ...
       begin
        ...
        begin
                showmessage('Caro Usuario, favor informar o numero da OS De Ofício.' );
                DBEOf.setfocus;
                exit;
        end;
...
{        if not (tbvisitasfiscal3.IsNull) and not (tbvisitasfiscal2.isnull) and not (tbvisitasfiscal3.isnull) and not tbvisitasporte = 'GRANDE' then
        begin
            showmessage('Não permitido 3 (tres) autoridades por inspecao!');
            exit;
        end;
 }
...
{     end;}
       ...
       begin
           ...
           showmessage('Atencao! Terceiro fiscal Não autorizado para esta OS');
           dbcordem.SetFocus;
           exit
       end;
                      ...
                      begin
                          ...
                          begin
                              ...
                              showmessage('Atencao! A Ordem de Requerimento: ' + tbreqos.AsString+' Não foi baixada pela certidao emitida. Necessorio reportar a Gerencia');
                          end;
                      end
                      else
                      begin
                          ...
                          begin
                              ...
                              begin
                                  showmessage('Não pode ser emitida certidao para Ordem de Requerimento: ' + tbreqos.AsString+' ja baixada.');
                                  DBCtipo.SetFocus;
                                  exit;
                              end;
                          end;
                  ...
                  begin
                    showmessage('Requerimento ainda Não a uma OS encaminhada pelo titular de vigilancia');
                    dbeos.SetFocus;
                    exit;
                  end;
                end
       ...
       begin
           ...
           exit
                ...
                begin
                    ...
                    begin
                        showmessage('Atencao! OS Não direcionada a inspecao. Impossovel continuar');
                        dbeof.SetFocus;
                        exit;
                    end;
...
//                else  showmessage('Achei protocolo '+inttostr(tbauxplantaprotocolo.value)+'  '+'regulado '+inttostr(tbauxplantacodigo.value))
            ...
            begin
                ...
                begin
                    showmessage('N. de denuncia Não encontrado.');
                    dbeden.SetFocus;
                    exit;
                end;
            end;
...
//    =======================
     ...
     begin
            ...
            if (daysbetween(date,tbvisitasdt_visita.value) > 1) then
               ...
               begin
                    if not tbhistorico.Eof then tbhistorico.Delete else showmessage('Especificacoes Não digitadas para este Documento!');
                    if not tbvisitas.Eof   then tbvisitas.Delete   else showmessage('Não Ha Documento a ser Excluido!');
...
begin
 showmessage('Alvara Não requerido para o exercicio!');
 exit;
end;

if tbalvaralibera.value <> true then
begin
 showmessage('Alvara Não liberado para o exercicio!');
 exit;
end;
painel:= 'alvara';
...
{        if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
        begin
                 showmessage('Usuario Não autorizado a liberacao de alvara provisorio!');
                 exit;
        end;    }
...
begin
     ...
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
             //    if not tbalvara.Eof then tbalvara.Delete else showmessage('Não Ha Alvara Solicitado para este Contribuinte!');
                   ...
                   else showmessage('Não Ha Alvara Solicitado para este Contribuinte!');
             end
...
{if (tbalvaratipo.Value = true) and (frmlogin.c_grp <> 'ADM') then
begin
 showmessage('Usuario Não autorizado a emitir alvara provisorio!');
 exit;
end;         }
  tbrt.First;
  while not tbrt.Eof do
...
begin
 showmessage('Exercicio Não requerido!');
 exit;
end;

if (tbalvlib.eof) and (tbalvlib.bof)  then
begin
 showmessage('Não existe atividade liberada para o exercicio!');
 exit;
end;
...
if FaltaRt <> '' then
begin
  ShowMessage('Os CNAEs abaixo constam no alvara mas Não estao no cadastro do contribuinte. Favor ajustar:' + #13#10 + FaltaRt);
  Exit;
end;
...
begin
    showmessage('Alvara Não Emitido!');
    exit;
end;
...
begin
  ...
  begin
      ...
      begin
          ...
          begin
             showmessage('Ha pelo menos uma atividade economica cadastrada com a subclasse Não informada. Selecione alterar e complete o cadastro antes de incluir outra atividade economica!');
             exit;
          end;
          tbauxrt.Next;
      end;
 ...
 {     if inttostr(tbalvaraexercicio.Value) <> formatdatetime('yyyy',tbalvlibData.Value) then
  ...
  begin
      ...
      //                 if (tbvisitaslibera.Value = 'Liberado') or (tbvisitaslibera.Value = 'Alguns CNAEs') then showmessage('ok');
                         ...
                         begin
                             showmessage('Atencao! A Autoridade Sanitaria relatou Não conformidade na inspecao')
                         end;
                         digitado := true;
                     end;
                 end;
          ...
          if digitado = false then showmessage ('Atencao!!! Documento de liberacao da atividade economica Não localizado. Click em Ok para continuar ...');
...
begin
     ...
     begin
             ...
             begin
                 if not tbalvaradt_emite.isnull then
                 begin
                     showmessage('Não a possovel excluir. Alvara ja emitido!');
                     exit;
                 end;
                 if not tbalvlib.Eof then tbalvlib.Delete else showmessage('Não Ha Atividade Liberada para este Contribuinte!');
             end
             else showmessage('Exclusao Não autorizada');
     end;
end;
...
{      if (frmlogin.c_grp = 'CAD') or (frmlogin.c_grp = 'CON') then
      begin
        if tbveiculoautorizacao.Value = true then
          begin
            showmessage('Usuario Não autorizado a liberacao veicular!');
            exit;
          end;
      end;}
        tbveiculo.post;
...
begin
     ...
     begin
             if (frmlogin.c_grp = 'ADM') then
             begin
                 if not tbveiculo.Eof then tbveiculo.Delete else showmessage('Não Ha Alvara Veicular Solicitado para este Contribuinte!');
             end
             else showmessage('Exclusao Não autorizada');
     end;
     btimprime.setfocus;
...
begin
    ...
    begin
       showmessage('Alvara Veicular Não requerido!');
       exit;
    end;

    if tbveiculoautorizacao.Value <> true then
    begin
      showmessage('Alvara Veicular Não autorizado!');
      exit;
    end;
...
//        showmessage('Atencao!!! Conferir o numero do processo (EM VERMELHO) que esta tramitando/consultanto');
    ...
    begin;
        showmessage('Atencao, salvar a edicao antes de mudar de aba. Os dados Não salvos serao perdidos. Cancelando a operacao...');
        tbtramitacao.Cancel;
        tbplanta.cancel;
        tbveiculo.Cancel;
        tbalvlib.Cancel;
  ...
  begin
    ...
    begin
        ...
        begin
           ...
           showmessage('A carga do processo Não esta com o usuario');
        end;
    end;
  end;
end;
...
begin
    showmessage('funcao Não permitida para o perfil');
end;
...
begin
  if (tbtramitacao.Bof) and (tbtramitacao.Eof) then showmessage('Não existe tramitacao pra este regulado') else
  begin
    ...
    begin
      ...
      else showmessage('O Registro selecionado Não a uma Tramitacao. a uma juntada de documentos');
    end;
  end;
end;
...
begin
  ...
  begin
   if (frmlogin.c_grp = 'ADM') or  (frmlogin.c_grp = 'CAD') then
   begin
     if tbtramitacaojuntada.IsNull then showmessage('O Registro selecionado Não a uma Juntada de documentos. a uma tramitacao') else
     begin
        painel := 'protocolo';
...
begin
  ...
  begin
    ShowMessage('Tabela SIMAUX Não encontrada. Execute a validacao na aba Complementar primeiro.');
    Exit;
  end;
  ...
  try
    ...
    begin
      ShowMessage('Cadastro Não localizado no SIM. Verifique inscricao municipal e CNPJ/CPF.');
      Exit;
    end;
