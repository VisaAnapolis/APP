(() => {
  ...
  function renderDetail(reg) {
    ...
    // ✅ GOOGLE MAPS (logo após o bairro)
    if (els.dMaps) {
      const enderecoMaps = endParts.join(", ").trim(); // logradouro + complemento
    ...
    if (els.atividadesList) {
      ...
      if (atvs.length === 0) {
        ...
        for (const a of atvs) {
          ...
          const linha = [a.atividade ? a.atividade : null, a.equipe ? `Equipe: ${a.equipe}` : null, a.complexidade ? `Complexidade: ${a.complexidade}` : null]
            .filter(Boolean)
            .join(" · ");
    ...
    if (els.inspecoesList) {
      ...
      if (insps.length === 0) {
        ...
        for (const v of insps) {
          ...
          badge.className = "tag";
          badge.textContent =
            v.pz_retorno !== undefined && v.pz_retorno !== null ? `Prazo: ${v.pz_retorno} dia(s)` : "Prazo: —";
